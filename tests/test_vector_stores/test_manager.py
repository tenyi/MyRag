...
class TestVectorStoreManager:
    ...
    async def manager(self, temp_db_path):
        """建立測試用的向量儲存管理器"""
        manager = VectorStoreManager(default_store_type=VectorStoreType.LANCEDB)
    ...
    async def test_create_vector_store_manager(self, temp_db_path):
        ...
        manager = await create_vector_store_manager(
            store_type=VectorStoreType.LANCEDB, db_path=temp_db_path
        )
    ...
    async def test_vector_operations(self, manager):
        ...
        search_result = await manager.search_vectors(
            collection_name=collection_name, query_vector=query_vector, k=2
        )
        ...
        batch_results = await manager.batch_search_vectors(
            collection_name=collection_name, query_vectors=query_vectors, k=1
        )
        ...
        updated_data = await manager.get_vector_by_id(
            collection_name=collection_name, vector_id="vec1"
        )
        ...
        success = await manager.delete_vectors(
            collection_name=collection_name, ids=["vec2", "vec3"]
        )
        assert success

        # 驗證刪除
        deleted_data = await manager.get_vector_by_id(
            collection_name=collection_name, vector_id="vec2"
        )
    ...
    async def test_hybrid_search_manager(self, manager):
        ...
        search_filter = SearchFilter(conditions={}, score_threshold=0.1)
    ...
    async def test_index_optimization_manager(self, manager):
        ...
        success = await manager.optimize_index(
            collection_name=collection_name, optimization_params={"index_type": "FLAT"}
        )
    ...
    async def test_multiple_stores(self, temp_db_path):
        ...
        store1 = await manager.get_store(
            VectorStoreType.LANCEDB, store_id="store1", db_path=temp_db_path + "_1"
        )

        store2 = await manager.get_store(
            VectorStoreType.LANCEDB, store_id="store2", db_path=temp_db_path + "_2"
        )
