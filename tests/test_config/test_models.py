...
class TestLLMConfig:
    ...
    def test_valid_llm_config(self):
        """測試有效的 LLM 配置"""
        config = LLMConfig(
            type=LLMType.OPENAI_CHAT, model="gpt-4", api_key="test-key", temperature=0.5
        )
    ...
    def test_invalid_temperature(self):
        ...
        with pytest.raises(ValidationError):
            LLMConfig(
                type=LLMType.OPENAI_CHAT, model="gpt-4", temperature=3.0  # 超出範圍
            )
...
class TestEmbeddingConfig:
    ...
    def test_invalid_batch_size(self):
        ...
        with pytest.raises(ValidationError):
            EmbeddingConfig(
                type=EmbeddingType.BGE_M3, model="BAAI/bge-m3", batch_size=0  # 無效值
            )
...
class TestGraphRAGConfig:
    ...
    def test_minimal_config(self):
        ...
        config = GraphRAGConfig(
            models={
                "default_llm": LLMConfig(type=LLMType.OPENAI_CHAT, model="gpt-4"),
    ...
    def test_get_llm_config(self):
        """測試取得 LLM 配置"""
        llm_config = LLMConfig(type=LLMType.OPENAI_CHAT, model="gpt-4")
    ...
    def test_default_model_configs(self):
        ...
        config = GraphRAGConfig(
            models={"my_llm": llm_config, "my_embedding": embedding_config},
            vector_store=VectorStoreConfig(
                type=VectorStoreType.LANCEDB, uri="./data/lancedb"
            ),
        )
