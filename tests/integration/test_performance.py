...
class TestPerformanceBenchmarks:
    ...
    def test_document_processing_performance(self, performance_config):
        ...
        for doc_count in performance_config["document_counts"]:
            ...
            for i in range(doc_count):
                doc_content = TestDataGenerator.generate_chinese_text(1000)  # 1000字文件
            ...
            with PerformanceTimer() as timer:
                ...
                for doc in test_documents:
                    # 模擬文本分塊
                    chunks = [doc["content"][i:i+500] for i in range(0, len(doc["content"]), 450)]
                    processed_doc = {
                        **doc,
                        "chunks": chunks,
            ...
            results[doc_count] = {
                ...
                "avg_chunks_per_doc": sum(doc["chunk_count"] for doc in result) / len(result)
            }
        ...
        # 驗證擴展性
        assert results[100]["docs_per_second"] >= results[10]["docs_per_second"] * 0.5, "處理速度下降過多"
    ...
    def test_embedding_generation_performance(self, performance_config):
        ...
        for batch_size in performance_config["batch_sizes"]:
            for dimension in performance_config["vector_dimensions"]:
                ...
                # 驗證效能要求
                if batch_size == 1:
                    assert processing_time < 1.0, f"單個向量生成過慢: {processing_time}s"
                elif batch_size <= 10:
                    assert processing_time < 5.0, f"小批次向量生成過慢: {processing_time}s"
                else:
                    assert processing_time < 30.0, f"大批次向量生成過慢: {processing_time}s"
    ...
    def test_memory_usage_performance(self, performance_config):
        ...
        for data_size in data_sizes:
            ...
            results[data_size] = {
                ...
                "memory_per_item_kb": (memory_used * 1024) / data_size if data_size > 0 else 0
            }
            
            # 驗證記憶體使用
            memory_limit = performance_config["memory_limit_mb"]
            assert memory_used < memory_limit, f"記憶體使用超限: {memory_used}MB > {memory_limit}MB"
    ...
    def test_end_to_end_performance(self, temp_dir, performance_config):
            ...
            with PerformanceTimer() as embedding_timer:
                ...
                for doc in processed_docs:
                    ...
                    doc["chunk_embeddings"] = [
                        TestDataGenerator.generate_vector(768) 
                        for _ in doc["chunks"]
                    ]
        ...
        performance_report = {
            ...
            "stage_breakdown": {
                "processing": (processing_timer.duration or 0.1) / (total_timer.duration or 1.0),
                "embedding": (embedding_timer.duration or 0.1) / (total_timer.duration or 1.0),
                "indexing": (indexing_timer.duration or 0.1) / (total_timer.duration or 1.0),
...
class TestStressTests:
    ...
    def test_high_concurrency_stress(self):
        ...
        average_response_time = timer.duration / concurrent_requests if timer.duration else 0
        
        assert success_rate > 0.95, f"成功率過低: {success_rate:.2%}"
        assert len(errors) < concurrent_requests * 0.05, f"錯誤過多: {len(errors)}"
        assert average_response_time < 1.0, f"平均響應時間過長: {average_response_time}s"
