...
from tests.test_utils import MockFactory, PerformanceTimer, TestDataGenerator
...
try:
    ...
    from src.chinese_graphrag.processors.chinese_text_processor import (
...
"""
    ...
    def test_full_indexing_pipeline(
        self, temp_workspace, sample_documents, mock_config
    ):
        """測試完整索引管道"""
        with PerformanceTimer() as timer:
            ...
            for doc in processed_docs:
                ...
                doc["chunk_embeddings"] = [
                    embedding_service.encode(chunk) for chunk in doc["chunks"]
                ]
            ...
            for doc in processed_docs:
                ...
                for i, (chunk, chunk_embedding) in enumerate(
                    zip(doc["chunks"], doc["chunk_embeddings"])
                ):
                    vector_store.add_document(
                        {
                            ...
                            "metadata": {"parent_doc": doc["id"], "chunk_index": i},
                        }
                    )
            ...
            indexer.get_statistics.return_value = {
                ...
                + sum(len(doc["chunks"]) for doc in processed_docs),
...
class TestEndToEndQuerying:
    ...
    def test_chinese_query_processing(self, indexed_system):
        ...
        for query in test_queries:
            ...
            processed_query = {
                "original": query,
                "intent": (
                    "定義查詢" if "什麼是" in query or "如何" in query else "比較查詢"
                ),
    ...
    def test_semantic_search(self, indexed_system):
        ...
        vector_store.search.return_value = [
            {"id": "doc_ai", "score": 0.95, "metadata": {"title": "人工智慧概述"}},
            {"id": "doc_ml", "score": 0.87, "metadata": {"title": "機器學習指南"}},
        ]
...
class TestSystemIntegration:
    ...
    def test_indexing_and_querying_integration(self, temp_dir):
        ...
        # 階段2：執行查詢
        query_engine = Mock()
        test_queries = ["什麼是人工智慧？", "機器學習和深度學習的關係？"]
