...
class TestChineseTextProcessor:
    ...
    def test_segment_text_no_pos_filter(self):
        """測試不過濾詞性的分詞"""
        text = "我很快樂地走在路上"
        words = self.processor.segment_text(text, keep_pos=False, remove_stopwords=False)
    ...
    def test_extract_keywords_basic(self):
        ...
        assert all('word' in kw and 'frequency' in kw and 'weight' in kw for kw in keywords)
    ...
    def test_get_text_statistics_comprehensive(self):
        ...
        expected_keys = [
            ...
            'chinese_char_count', 'english_word_count', 'punctuation_count',
    ...
    def test_evaluate_text_quality_good_text(self):
        ...
        expected_keys = [
            'length', 'chinese_ratio', 'sentence_structure', 
            'vocabulary_richness', 'punctuation', 'coherence', 'special_chars'
        ]
    ...
    def test_extract_named_entities_candidates_basic(self):
        ...
        # 應該包含人名、地名、日期、電話等
        expected_patterns = ['李明', '上海', '華東師範大學', '2023年10月15日', '15912345678']
        found_entities = []
    ...
    def test_extract_named_entities_candidates_confidence_threshold(self):
        ...
        # 測試不同信心度閾值
        candidates_low = self.processor.extract_named_entities_candidates(text, min_confidence=0.1)
        candidates_high = self.processor.extract_named_entities_candidates(text, min_confidence=0.9)
    ...
    def test_entity_recognition_integration(self):
        ...
        expected_types = {'人名', '地名', '機構名', 'phone', 'email', 'url', 'date', 'number'}
        ...
        for candidate in candidates:
            ...
            assert candidate['text'] in extracted_text or extracted_text in candidate['text']
