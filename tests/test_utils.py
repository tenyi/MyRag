...
class TestDataGenerator:
    ...
    def generate_chinese_text(length: int = 100) -> str:
        ...
        chinese_words = [
            ...
            "電腦視覺", "資料科學", "演算法", "模型訓練", "特徵提取",
            "分類", "迴歸", "聚類", "降維", "優化", "梯度下降",
            "反向傳播", "卷積", "池化", "全連接", "激活函數", "損失函數",
            "準確率", "召回率", "精確率", "F1分數", "交叉驗證", "過擬合"
        ]
...
class TestAssertions:
    ...
    def assert_vector_format(vector: List[float], expected_dimension: int):
        ...
        assert len(vector) == expected_dimension, f"向量維度不匹配: {len(vector)} != {expected_dimension}"
...
class PerformanceTimer:
    ...
    def assert_duration_less_than(self, max_seconds: float):
        ...
        assert self.duration < max_seconds, f"執行時間過長: {self.duration:.3f}s > {max_seconds}s"
