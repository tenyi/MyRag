...
class TestCommunity:
    ...
    def test_title_validation(self):
        ...
        with pytest.raises(ValidationError):
            Community(title="", level=0, summary="摘要", full_content="內容")
        ...
        # 太長的標題應該失敗
        with pytest.raises(ValidationError):
            Community(title="x" * 201, level=0, summary="摘要", full_content="內容")

        # 正常標題應該成功，並且會被 strip
        community = Community(
            title="  正常標題  ", level=0, summary="摘要", full_content="內容"
        )
    ...
    def test_level_validation(self):
        ...
        # 正數應該成功
        community = Community(
            title="社群", level=5, summary="摘要", full_content="內容"
        )
        assert community.level == 5

        # 零應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容"
        )
        assert community.level == 0

        # 負數應該失敗
        with pytest.raises(ValidationError):
            Community(title="社群", level=-1, summary="摘要", full_content="內容")

    def test_summary_validation(self):
        """測試摘要驗證"""
        # 空摘要應該失敗
        with pytest.raises(ValidationError):
            Community(title="社群", level=0, summary="", full_content="內容")
        ...
        # 正常摘要應該成功，並且會被 strip
        community = Community(
            title="社群", level=0, summary="  正常摘要  ", full_content="內容"
        )
    ...
    def test_full_content_validation(self):
        ...
        # 空內容應該失敗
        with pytest.raises(ValidationError):
            Community(title="社群", level=0, summary="摘要", full_content="")
        ...
        # 正常內容應該成功，並且會被 strip
        community = Community(
            title="社群", level=0, summary="摘要", full_content="  正常內容  "
        )
    ...
    def test_rank_validation(self):
        ...
        # 正常範圍應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", rank=0.5
        )
        assert community.rank == 0.5

        # 邊界值應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", rank=0.0
        )
        assert community.rank == 0.0

        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", rank=1.0
        )
        ...
        with pytest.raises(ValidationError):
            Community(
                title="社群", level=0, summary="摘要", full_content="內容", rank=-0.1
            )

        with pytest.raises(ValidationError):
            Community(
                title="社群", level=0, summary="摘要", full_content="內容", rank=1.1
            )
    ...
    def test_size_validation(self):
        ...
        # 正數應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", size=10
        )
        assert community.size == 10

        # 零應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", size=0
        )
        ...
        with pytest.raises(ValidationError):
            Community(
                title="社群", level=0, summary="摘要", full_content="內容", size=-1
            )
    ...
    def test_density_validation(self):
        ...
        # 正常範圍應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", density=0.5
        )
        assert community.density == 0.5

        # 邊界值應該成功
        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", density=0.0
        )
        assert community.density == 0.0

        community = Community(
            title="社群", level=0, summary="摘要", full_content="內容", density=1.0
        )
        ...
        with pytest.raises(ValidationError):
            Community(
                title="社群", level=0, summary="摘要", full_content="內容", density=-0.1
            )

        with pytest.raises(ValidationError):
            Community(
                title="社群", level=0, summary="摘要", full_content="內容", density=1.1
            )
    ...
    def test_get_summary_preview(self):
        ...
        short_summary = "短摘要"
        community = Community(
            title="社群", level=0, summary=short_summary, full_content="內容"
        )
        ...
        community = Community(
            title="社群", level=0, summary=long_summary, full_content="內容"
        )
