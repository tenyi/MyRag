...
class TestTextUnit:
    ...
    def test_create_text_unit_with_required_fields(self):
        """測試使用必要欄位建立文本單元"""
        text_unit = TextUnit(text="這是測試文本", document_id="doc-123", chunk_index=0)
    ...
    def test_position_validation(self):
        ...
        with pytest.raises(ValidationError):
            TextUnit(
                text="文本", document_id="doc-123", chunk_index=0, start_position=-1
            )
    ...
    def test_embedding_validation(self):
        ...
        embedding = np.array([0.1, 0.2, 0.3])
        text_unit = TextUnit(
            text="文本", document_id="doc-123", chunk_index=0, embedding=embedding
        )
        assert np.array_equal(text_unit.embedding, embedding)
    ...
    def test_token_count_validation(self):
        ...
        # 正數應該成功
        text_unit = TextUnit(
            text="文本", document_id="doc-123", chunk_index=0, token_count=100
        )
        assert text_unit.token_count == 100

        # 零應該成功
        text_unit = TextUnit(
            text="文本", document_id="doc-123", chunk_index=0, token_count=0
        )
        assert text_unit.token_count == 0

        # 負數應該失敗
        with pytest.raises(ValidationError):
            TextUnit(text="文本", document_id="doc-123", chunk_index=0, token_count=-1)
