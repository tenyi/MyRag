...
from chinese_graphrag.models import Entity, Relationship, Community, TextUnit, Document
from chinese_graphrag.config import GraphRAGConfig, VectorStoreConfig, VectorStoreType
from chinese_graphrag.query import (
    LLMManager, LLMConfig, LLMProvider, TaskType,
    ChineseQueryProcessor, QueryType, QueryIntent,
    GlobalSearchEngine, LocalSearchEngine, QueryEngine,
    QueryEngineConfig
)
...
def mock_indexer():
    ...
    entities = {
        ...
        "e3": Entity(id="e3", name="人工智慧", type="技術", description="機器學習相關技術")
    }
...
def llm_configs():
    ...
    return [
        LLMConfig(
            ...
            task_types=[TaskType.GLOBAL_SEARCH, TaskType.LOCAL_SEARCH, TaskType.GENERAL_QA]
        )
    ]
...
class TestChineseQueryProcessor:
    ...
    def test_process_relationship_query(self, processor):
        ...
        assert analysis.query_type in [QueryType.RELATION_SEARCH, QueryType.LOCAL_SEARCH]
    ...
    def test_process_global_query(self, processor):
        ...
        assert analysis.intent in [QueryIntent.INFORMATION_SEEKING, QueryIntent.EXPLANATION]
        assert analysis.confidence > 0
...
class TestGlobalSearchEngine:
    ...
    async def test_global_search(self, global_search_engine, mock_indexer):
        """測試全域搜尋"""
        from chinese_graphrag.query.processor import QueryAnalysis, QueryType, QueryIntent
...
class TestQueryEngine:
    ...
    @pytest.fixture
    def query_engine(self, query_engine_config, graphrag_config, mock_indexer, mock_vector_store):
        """查詢引擎 fixture"""
        return QueryEngine(
...
class TestIntegration:
    ...
    def test_query_result_serialization(self, query_engine_config, graphrag_config, mock_indexer, mock_vector_store):
        """測試查詢結果序列化"""
