...
class TestGraphRAGIndexer:
    ...
    def config(self):
        ...
        return GraphRAGConfig(
            ...
            vector_store=VectorStoreConfig(
                type=VectorStoreType.LANCEDB, uri="./test_data/lancedb"
            ),
        )
    ...
    async def test_entity_and_relationship_extraction(
        self, indexer, sample_documents, mocker
    ):
        """測試實體和關係提取的整合流程"""
        import json
        import uuid
            ...
            "entities": [
                {
                    "name": "張三博士",
                    "type": "人物",
                    "description": "在台灣大學進行研究的博士",
                },
                {
                    "name": "台灣大學",
                    "type": "組織",
                    "description": "台灣的一所頂尖大學",
                },
            ],
            "relationships": [
                {
        ...
        # 模擬 create_llm 函數
        mocker.patch("chinese_graphrag.llm.create_llm", return_value=mock_llm)

        # 建立文本單元
        text_units = await indexer._create_text_units(sample_documents)

        # 執行實體和關係提取
        entities, relationships = await indexer._extract_entities_and_relationships(
            text_units
        )
    ...
    async def test_create_embeddings(self, indexer, sample_documents, mocker):
        ...
        # 準備模擬資料
        from chinese_graphrag.models import Community, Entity, TextUnit
        ...
        entities = [
            Entity(
                id="e1", name="張三博士", type="人物", text_units=[text_units[0].id]
            ),
            Entity(
                id="e2", name="台灣大學", type="組織", text_units=[text_units[0].id]
            ),
        ]
        ...
        mocker.patch.object(indexer.embedding_manager, "embed_texts", mock_embed_texts)
        ...
        mocker.patch.object(
            indexer.vector_store_manager, "store_text_unit", mock_store_text_unit
        )
        mocker.patch.object(
            indexer.vector_store_manager, "store_entity", mock_store_entity
        )
        mocker.patch.object(
            indexer.vector_store_manager, "store_community", mock_store_community
        )
        ...
        for i, community in enumerate(communities):
            ...
            np.testing.assert_array_equal(
                community.embedding, community_result.embeddings[i]
            )
    ...
    async def test_full_indexing_workflow(self, indexer, sample_documents, mocker):
        ...
        mock_process_documents = mocker.patch.object(
            indexer, "_process_documents", return_value=sample_documents
        )
        ...
        mock_create_text_units = mocker.patch.object(
            indexer, "_create_text_units", return_value=text_units
        )
        ...
        mock_detect = mocker.patch.object(
            indexer, "_detect_communities", return_value=communities
        )

        # 模擬嵌入建立
        mock_create_embeddings = mocker.patch.object(
            indexer, "_create_embeddings", return_value=None
        )

        # 模擬結果儲存
        mock_save_results = mocker.patch.object(
            indexer, "_save_results", return_value=None
        )
