...
from src.chinese_graphrag.embeddings import (  # 快取相關; 管理器
    CacheEntry,
    DiskCache,
    EmbeddingManager,
    LFUStrategy,
...
class TestEmbeddingCache:
    ...
    async def test_memory_cache_lru_eviction(self):
        """測試 LRU 淘汰策略"""
        cache = MemoryCache(
            max_size_mb=0.003, strategy=LRUStrategy()
        )  # 很小的快取，約3KB
    ...
    async def test_multi_level_cache(self, cache_entry):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            cache = MultiLevelCache(
                memory_cache_mb=1, disk_cache_mb=10, cache_dir=temp_dir
            )
    ...
    def test_cache_factory(self):
        ...
        # 測試磁碟快取
        with tempfile.TemporaryDirectory() as temp_dir:
            disk_cache = create_embedding_cache(
                "disk", cache_dir=temp_dir, max_size_mb=10
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            multi_cache = create_embedding_cache(
                "multi_level", memory_cache_mb=1, disk_cache_mb=10, cache_dir=temp_dir
            )
    ...
    async def test_manager_with_cache(self, mock_embedding_service):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manager = EmbeddingManager(
                embedding_service=mock_embedding_service, cache=cache
            )
