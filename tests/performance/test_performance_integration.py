...
class TestPerformanceIntegration:
    ...
    async def test_cost_optimization(self, optimizer_manager):
        ...
        assert "recommended_model" in recommendation or "recommendation" in recommendation
    ...
    async def test_performance_monitoring(self, optimizer_manager):
        ...
        # 檢查歷史資料
        history = optimizer_manager.performance_monitor.get_metrics_history(1)  # 1分鐘內
    ...
    async def test_config_persistence(self, temp_storage, optimization_config):
        ...
        assert loaded_manager.config.query_cache_enabled == optimization_config.query_cache_enabled
        assert loaded_manager.config.cost_tracking_enabled == optimization_config.cost_tracking_enabled
    ...
    async def test_alert_handling(self, optimizer_manager):
        ...
        if optimizer_manager.batch_optimizer:
            # 批次大小應該已減少
            assert optimizer_manager.batch_optimizer.current_batch_size <= optimizer_manager.batch_optimizer.default_batch_size
        
        # 模擬高 CPU 使用警報
        original_workers = optimizer_manager.batch_optimizer.parallel_workers if optimizer_manager.batch_optimizer else 0
        await optimizer_manager._handle_alert("cpu_usage", 85.0, 80.0)
        
        # 檢查並行工作者數量是否已調整
        if optimizer_manager.batch_optimizer:
            assert optimizer_manager.batch_optimizer.parallel_workers <= original_workers
