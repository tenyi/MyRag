...
class InputConfig(BaseModel):
    ...
    supported_formats: List[str] = Field(
        default=["txt", "pdf", "docx", "md"], description="支援的檔案格式"
    )
...
class IndexingConfig(BaseModel):
    ...
    entity_types: List[str] = Field(
        default=["organization", "person", "geo", "event"], description="實體類型"
    )
    ...
    enable_hierarchical_communities: bool = Field(
        True, description="是否啟用層次化社群檢測"
    )
...
class ModelSelectionConfig(BaseModel):
    ...
    default_embedding: str = Field(
        "ollama_embedding_model", description="預設 Embedding 模型"
    )
    ...
    fallback_models: Dict[str, str] = Field(
        default_factory=dict, description="備用模型映射"
    )
...
class LoggingConfig(BaseModel):
    ...
    format: str = Field(
        "%(asctime)s - %(name)s - %(levelname)s - %(message)s", description="日誌格式"
    )
    file_path: Optional[str] = Field(
        "./logs/chinese_graphrag.log", description="日誌檔案路徑"
    )
...
class GraphRAGConfig(BaseModel):
    ...
    chinese_processing: ChineseProcessingConfig = Field(
        default_factory=ChineseProcessingConfig, description="中文處理配置"
    )
    ...
    storage: StorageConfig = Field(
        default_factory=StorageConfig, description="儲存配置"
    )

    # 並行處理配置
    parallelization: ParallelizationConfig = Field(
        default_factory=ParallelizationConfig, description="並行處理配置"
    )

    # 模型選擇配置
    model_selection: ModelSelectionConfig = Field(
        default_factory=ModelSelectionConfig, description="模型選擇配置"
    )

    # 日誌配置
    logging: LoggingConfig = Field(
        default_factory=LoggingConfig, description="日誌配置"
    )

    # 監控配置
    monitoring: MonitoringConfig = Field(
        default_factory=MonitoringConfig, description="監控配置"
    )

    # 效能配置
    performance: PerformanceConfig = Field(
        default_factory=PerformanceConfig, description="效能配置"
    )
    ...
    experimental: ExperimentalConfig = Field(
        default_factory=ExperimentalConfig, description="實驗性功能配置"
    )
