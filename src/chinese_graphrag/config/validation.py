...
class ConfigValidator:
    ...
    def _validate_embedding_config(self, name: str, config: EmbeddingConfig) -> None:
        ...
        # 檢查 API 金鑰（對於需要的提供者）
        if config.type in [
            EmbeddingType.OPENAI_EMBEDDING,
            EmbeddingType.AZURE_OPENAI_EMBEDDING,
        ]:
            if not config.api_key:
    ...
    def _validate_business_logic(self, config: GraphRAGConfig) -> None:
        ...
        if (
            config.query.similarity_threshold < 0
            or config.query.similarity_threshold > 1
        ):
...
class DefaultConfigProvider:
    ...
    def _apply_processing_defaults(config: Dict[str, Any]) -> None:
        ...
        if "chunks" not in config:
            config["chunks"] = {"size": 1000, "overlap": 200, "strategy": "token"}
