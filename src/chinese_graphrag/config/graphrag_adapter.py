...
class GraphRAGConfigAdapter:
                ...
                if hasattr(model_config, 'type'):
                    if model_config.type == "openai":
                        ...
                        models["ollama_chat_model"] = {
                            ...
                            "api_base": getattr(model_config, 'base_url', 'http://localhost:11434')
                        }
            ...
            if hasattr(embedding_config, 'type'):
                if embedding_config.type == "bge_m3":
                    ...
                    models["default_embedding_model"] = {
                        ...
                        "api_key": "${GRAPHRAG_API_KEY}",
                        "model": getattr(embedding_config, 'model', 'text-embedding-3-small')
                    }
    ...
    def create_graphrag_config_file(self, output_path: Union[str, Path]) -> Path:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            yaml.dump(graphrag_config, f, default_flow_style=False, allow_unicode=True, indent=2)
        ...
        if validation_result.warnings:
            ...
            for warning in validation_result.warnings:
                logger.warning(f"  - {warning['section']}.{warning['key']}: {warning['message']}")
    ...
    def validate_chinese_config(self) -> "ValidationResult":
        ...
        """
        from .graphrag_validator import ValidationResult, ValidationLevel
        ...
        # 驗證向量存儲配置
        if hasattr(self.chinese_config, 'vector_store') and self.chinese_config.vector_store:
    ...
    def validate_converted_config(self, config_dict: Dict[str, Any]) -> "ValidationResult":
        """驗證轉換後的 GraphRAG 配置
