...
class GraphRAGConfigValidator:
    ...
    def _validate_models_section(
        ...
        if not isinstance(models, dict):
            result.add_issue(
                ValidationLevel.ERROR, "models", "invalid_type", "models 必須是字典格式"
            )
            return
    ...
    def _validate_model_config(
        self, model_name: str, model_config: Dict[str, Any], result: ValidationResult
    ):
        """驗證單個模型配置"""
    ...
    def _validate_input_section(self, config: Dict[str, Any], result: ValidationResult):
        ...
        if "input" not in config:
            result.add_issue(
                ValidationLevel.ERROR, "input", "missing_section", "缺少 input 配置區段"
            )
            return
    ...
    def _validate_workflows_section(
        self, config: Dict[str, Any], result: ValidationResult
    ):
        """驗證工作流程配置區段"""
    ...
    def _validate_chinese_processing_section(
        self, config: Dict[str, Any], result: ValidationResult
    ):
        """驗證中文處理配置區段"""
    ...
    def _validate_performance_section(
        self, config: Dict[str, Any], result: ValidationResult
    ):
        """驗證效能配置區段"""
    ...
    def _check_storage_compatibility(
        self, config: Dict[str, Any], result: ValidationResult
    ):
        """檢查存儲相容性"""
        # 檢查存儲路徑是否衝突
    ...
    def _check_chinese_processing_compatibility(
        self, config: Dict[str, Any], result: ValidationResult
    ):
        """檢查中文處理相容性"""
