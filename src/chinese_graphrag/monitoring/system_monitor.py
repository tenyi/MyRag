...
class SystemMonitor:
    ...
    def _get_network_stats(self) -> Tuple[float, float]:
        ...
        # 計算傳輸速率（MB/s）
        sent_mb = (current_network.bytes_sent - self._network_baseline.bytes_sent) / (1024**2) / time_diff
        recv_mb = (current_network.bytes_recv - self._network_baseline.bytes_recv) / (1024**2) / time_diff
    ...
    def _check_system_alerts(self, stats: SystemStats) -> None:
        ...
        # 負載平均告警
        if stats.load_average and stats.load_average[0] > self.alert_thresholds.get("load_average", 100):
            alerts.append({
                "type": "load_high",
    ...
    def add_health_check(self, name: str, check_func: Callable[[], HealthCheck]) -> None:
        """添加健康檢查。
