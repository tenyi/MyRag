    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            if self._initial_net_io:
                ...
                if time_diff > 0:
                    net_sent_mb = (
                        current_net_io.bytes_sent - self._initial_net_io.bytes_sent
                    ) / (1024 * 1024)
                    net_recv_mb = (
                        current_net_io.bytes_recv - self._initial_net_io.bytes_recv
                    ) / (1024 * 1024)
            ...
            self.record_gauge(
                "system.memory_percent", memory.percent, {"unit": "percent"}
            )
            self.record_gauge(
                "system.disk_usage_percent", disk.percent, {"unit": "percent"}
            )
