...
class ErrorTracker:
    ...
    def track_error(
        ...
        with self.lock:
            ...
            error_hash = self._generate_error_hash(
                error_type, message, module, function
            )
    ...
    def _generate_error_hash(
        self, error_type: str, message: str, module: str, function: str
    ) -> str:
        """產生錯誤雜湊。"""
    ...
    def _cleanup_old_errors(self) -> None:
        ...
        expired_hashes = [
            error_hash
            for error_hash, error_record in self.errors.items()
        ...
        if len(self.errors) > self.max_errors:
            # 移除最舊的錯誤
            sorted_errors = sorted(self.errors.items(), key=lambda x: x[1].last_seen)
    ...
    def get_error_stats(self, hours: int = 1) -> ErrorStats:
        ...
        with self.lock:
            ...
            # 排序獲取最多的錯誤
            top_errors = sorted(recent_errors, key=lambda x: x.count, reverse=True)[:10]

            # 最近錯誤（按時間排序）
            recent_error_list = sorted(
                recent_errors, key=lambda x: x.last_seen, reverse=True
            )[:20]
                    ...
                    "errors": [
                        {
                            ...
                            "last_seen": (
                                error.last_seen.isoformat() if error.last_seen else None
                            ),
