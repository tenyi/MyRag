...
class CostBudget:
    ...
    def is_within_budget(self, additional_cost: float) -> bool:
        """檢查是否在預算範圍內"""
        return (
            self.current_daily_usage + additional_cost <= self.daily_limit
            and self.current_monthly_usage + additional_cost <= self.monthly_limit
        )
...
class ModelUsageTracker:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        """
        self.storage_path = (
            Path(storage_path) if storage_path else Path("logs/model_usage")
        )
        ...
        self._monthly_stats: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: defaultdict(float)
        )
    ...
    def record_usage(
        ...
        logger.debug(
            f"記錄模型使用: {model_name} - {task_type.value}, "
    ...
    def _update_stats(self, record: UsageRecord):
        ...
        self._realtime_stats["avg_latency"] = (
            self._realtime_stats["avg_latency"] * (total_requests - 1)
            + record.latency_ms
        ) / total_requests

        successful_requests = sum(1 for r in self._usage_records if r.success)
        self._realtime_stats["success_rate"] = (
    ...
    def get_usage_stats(
        ...
        with self._usage_lock:
            ...
            if model_name:
                filtered_records = [
                    r for r in filtered_records if r.model_name == model_name
                ]

            if task_type:
                filtered_records = [
                    r for r in filtered_records if r.task_type == task_type
                ]
            ...
            if filtered_records:
                ...
                avg_latency = sum(r.latency_ms for r in filtered_records) / len(
                    filtered_records
                )
                success_rate = sum(1 for r in filtered_records if r.success) / len(
                    filtered_records
                )
    ...
    def get_top_models(
        self, limit: int = 10, metric: str = "cost"
...
class CostOptimizer:
    ...
    def select_optimal_model(
        ...
        candidate_models = [
            ...
            and config.quality_score >= quality_requirement
        ]
        ...
        for config in candidate_models:
            # 計算綜合分數
            cost_score = 1 / (
                1 + config.cost_per_token * input_size
            )  # 成本越低分數越高
            ...
            performance_score = historical_performance.get(
                "avg_quality", config.quality_score
            )
    ...
    def _get_historical_performance(self, model_name: str) -> Dict[str, float]:
        ...
        avg_quality = sum(record.get("quality", 0.8) for record in history) / len(
            history
        )
        avg_latency = sum(record.get("latency", 1000) for record in history) / len(
            history
        )
        success_rate = sum(
            1 for record in history if record.get("success", True)
        ) / len(history)
    ...
    async def evaluate_response_quality(
        self, task_type: TaskType, input_text: str, output_text: str, model_name: str
    ) -> float:
        """評估回應品質
    ...
    def _generate_cost_suggestions(self) -> List[str]:
        ...
        for model_name, budget in self._budgets.items():
            ...
            if daily_usage_rate > 0.8:
                suggestions.append(
                    f"模型 {model_name} 日預算使用率過高 ({daily_usage_rate:.1%})"
                )
    ...
    def optimize_model_selection(self):
        ...
        for model_name, history in self._performance_history.items():
            if len(history) >= 10:  # 需要足夠的資料
                ...
                avg_quality = sum(record["quality"] for record in recent_history) / len(
                    recent_history
                )
                avg_latency = sum(record["latency"] for record in recent_history) / len(
                    recent_history
                )
                success_rate = sum(
                    1 for record in recent_history if record["success"]
                ) / len(recent_history)
                avg_cost = sum(record["cost"] for record in recent_history) / len(
                    recent_history
                )
        ...
        for model_name, analysis in performance_analysis.items():
            if model_name in self._model_configs:
                ...
                config.quality_score = (
                    0.7 * config.quality_score + 0.3 * analysis["avg_quality"]
                )
                config.latency_ms = (
                    0.7 * config.latency_ms + 0.3 * analysis["avg_latency"]
                )
