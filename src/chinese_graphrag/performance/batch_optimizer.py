...
from ..embeddings.gpu_acceleration import get_memory_optimizer, get_device_manager
...
class BatchStats:
    ...
    def update_batch_result(self, batch_size: int, processing_time: float, success: bool):
        """更新批次結果統計"""
        ...
        self.min_batch_processing_time = min(self.min_batch_processing_time, processing_time)
        self.max_batch_processing_time = max(self.max_batch_processing_time, processing_time)
        
        # 計算吞吐量
        if self.total_processing_time > 0:
            self.throughput_items_per_second = self.processed_items / self.total_processing_time
    
    def get_summary(self) -> Dict[str, Any]:
        """取得統計摘要"""
        success_rate = self.successful_batches / self.total_batches if self.total_batches > 0 else 0
...
class BatchOptimizer:
    ...
    def __init__(self, config: Optional[BatchProcessingConfig] = None):
        ...
        # 效能監控
        self._performance_history: List[Tuple[int, float]] = []  # (batch_size, processing_time)
    ...
    def _initialize_executors(self):
        ...
        if self.config.use_process_pool:
            ...
            logger.info(f"初始化執行緒池執行器，工作執行緒數: {self.config.max_workers}")
    ...
    def calculate_optimal_batch_size(self, 
                                   ...
                                   target_processing_time: Optional[float] = None) -> int:
        """計算最佳批次大小
        ...
        with self._batch_size_lock:
            ...
            available_memory_mb = (memory_stats.system_total * (1 - self.config.memory_threshold) - 
                                 memory_stats.process_used)
            
            if self.config.memory_limit_mb:
                available_memory_mb = min(available_memory_mb, self.config.memory_limit_mb)
            
            memory_based_batch_size = max(
                self.config.min_batch_size,
                min(self.config.max_batch_size, int(available_memory_mb / estimated_memory_per_item))
            )
            ...
            if self.config.adaptive_batch_size and self._performance_history:
                performance_adjusted_size = self._adjust_batch_size_by_performance(
                    ...
                    target_processing_time or self.config.performance_target_ms / 1000.0
                )
            else:
    ...
    def _adjust_batch_size_by_performance(self, base_batch_size: int, target_time: float) -> int:
        """基於效能歷史調整批次大小"""
        ...
        # 計算平均處理時間和批次大小的關係
        avg_time_per_item = sum(time / size for size, time in recent_history) / len(recent_history)
        
        # 基於目標時間計算理想批次大小
        ideal_batch_size = int(target_time / avg_time_per_item)
        
        # 使用調整因子進行漸進式調整
        adjustment = int((ideal_batch_size - base_batch_size) * self.config.batch_size_adjustment_factor)
        adjusted_size = base_batch_size + adjustment
        
        return max(self.config.min_batch_size, min(self.config.max_batch_size, adjusted_size))
    
    async def process_in_batches_async(self,
                                     items: List[Any],
        ...
        while processed_items < len(items):
            ...
            try:
                ...
                self.stats.update_batch_result(len(current_batch), processing_time, success)
                ...
                if (self.config.enable_gc and 
                    self.stats.total_batches - self._last_gc_batch >= self.config.gc_frequency):
                    gc.collect()
                ...
                # 錯誤處理和重試邏輯
                if self.config.enable_fallback and batch_size > self.config.min_batch_size:
                    logger.info(f"嘗試減小批次大小重試: {batch_size} -> {batch_size // 2}")
                    self.current_batch_size = max(self.config.min_batch_size, batch_size // 2)
                    continue
                else:
            ...
            self.stats.peak_memory_usage_mb = max(self.stats.peak_memory_usage_mb, current_memory)
        
        logger.info(f"異步批次處理完成，處理了 {processed_items} 個項目")
        return results
    
    def process_in_batches_parallel(self,
                                  items: List[Any],
                                  process_func: Callable,
                                  estimated_memory_per_item: int = 10,
                                  show_progress: bool = True) -> List[Any]:
        ...
        results = []
        executor = self._process_executor if self.config.use_process_pool else self._thread_executor
        
        if not executor:
            raise RuntimeError("執行器未初始化，請使用 with 語句或手動調用 _initialize_executors()")
        ...
        for future in as_completed(future_to_batch):
            ...
            try:
                ...
                if show_progress:
                    ...
                    logger.info(f"並行處理進度: {progress:.1f}% ({processed_batches}/{len(batches)})")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if self._performance_history:
            ...
            avg_processing_time = sum(time for _, time in recent_history) / len(recent_history)
            avg_batch_size = sum(size for size, _ in recent_history) / len(recent_history)
    ...
    def get_recommended_config(self, workload_type: str = "balanced") -> BatchProcessingConfig:
        """取得推薦的配置
