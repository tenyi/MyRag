...
from .performance_monitor import BenchmarkRunner, PerformanceMonitor
...
class OptimizationConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OptimizationConfig":
        ...
        if "batch" in data:
            ...
            config.max_batch_size = batch_config.get(
                "max_batch_size", config.max_batch_size
            )
            config.parallel_workers = batch_config.get(
                "parallel_workers", config.parallel_workers
            )
            config.memory_threshold_mb = batch_config.get(
                "memory_threshold_mb", config.memory_threshold_mb
            )

        if "query" in data:
            query_config = data["query"]
            config.query_cache_enabled = query_config.get(
                "cache_enabled", config.query_cache_enabled
            )
            config.cache_ttl_seconds = query_config.get(
                "cache_ttl_seconds", config.cache_ttl_seconds
            )
            config.cache_max_size = query_config.get(
                "cache_max_size", config.cache_max_size
            )
            config.preload_enabled = query_config.get(
                "preload_enabled", config.preload_enabled
            )

        if "cost" in data:
            cost_config = data["cost"]
            config.cost_tracking_enabled = cost_config.get(
                "tracking_enabled", config.cost_tracking_enabled
            )
            config.budget_limit_usd = cost_config.get(
                "budget_limit_usd", config.budget_limit_usd
            )
            config.quality_threshold = cost_config.get(
                "quality_threshold", config.quality_threshold
            )

        if "monitoring" in data:
            monitoring_config = data["monitoring"]
            config.monitoring_enabled = monitoring_config.get(
                "enabled", config.monitoring_enabled
            )
            config.monitoring_interval = monitoring_config.get(
                "interval", config.monitoring_interval
            )
            config.alert_thresholds = monitoring_config.get(
...
class OptimizerManager:
    ...
    async def _handle_alert(self, metric_name: str, value: float, threshold: float):
        ...
        if metric_name == "memory_usage" and self.batch_optimizer:
            ...
            logger.info(
                f"已調整批次大小以降低記憶體使用: {self.batch_optimizer.current_batch_size}"
            )
    ...
    async def optimize_batch_processing(
        ...
        result = await self.batch_optimizer.process_in_batches_async(
            items=items, process_func=batch_process_func, **kwargs
        )
    ...
    # 查詢優化介面
    async def optimize_query(self, query: str, query_func: callable, **kwargs) -> Any:
        """優化查詢處理
        ...
        result = await self.query_optimizer.optimize_query(
            query=query, query_func=query_func, context=kwargs
        )
    ...
    async def optimize_model_usage(
        self, model_name: str, input_tokens: int, operation_type: str = "inference"
    ) -> Dict[str, Any]:
        """優化模型使用
        ...
        # 記錄使用情況 - 使用正確的方法名稱和參數
        if (
            hasattr(self.cost_optimizer, "usage_tracker")
            and self.cost_optimizer.usage_tracker
        ):
            from .cost_optimizer import ModelType, TaskType

            task_type = (
                TaskType.EMBEDDING if operation_type == "embedding" else TaskType.QUERY
            )
        ...
        try:
            optimal_model = self.cost_optimizer.select_optimal_model(
                task_type=task_type, input_size=input_tokens
            )
            recommendation = {
    ...
    async def run_performance_benchmark(
        ...
        results = await self.benchmark_runner.run_comparative_benchmark(
            test_configs, iterations
        )
