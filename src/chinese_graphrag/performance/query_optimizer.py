...
class QueryStats:
    ...
    def _extract_query_pattern(self, query: str) -> str:
        """提取查詢模式"""
        # 簡單的模式提取：基於查詢長度和關鍵詞
        length_category = "short" if len(query) < 50 else "medium" if len(query) < 200 else "long"
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_cache_hit_time": self.cache_hit_time / self.cache_hits if self.cache_hits > 0 else 0,
            "avg_cache_miss_time": self.cache_miss_time / self.cache_misses if self.cache_misses > 0 else 0,
            "top_queries": dict(sorted(self.popular_queries.items(), key=lambda x: x[1], reverse=True)[:10]),
...
class QueryCache:
    ...
    def __init__(self, config: Optional[QueryCacheConfig] = None):
        ...
        # 語義快取（用於相似查詢）
        self._semantic_cache: Dict[str, Tuple[np.ndarray, Any, float]] = {}  # query -> (embedding, result, timestamp)
        
        logger.info(f"查詢快取初始化完成，記憶體快取大小: {self.config.memory_cache_size}")
    ...
    async def get(self, query: str, context: Optional[Dict[str, Any]] = None) -> Optional[Any]:
        ...
        with self._cache_lock:
            if cache_key in self._memory_cache:
                # 檢查是否過期
                if time.time() - self._cache_timestamps.get(cache_key, 0) < self.config.memory_cache_ttl:
                    # 移到最前面（LRU）
    ...
    async def put(self, query: str, result: Any, context: Optional[Dict[str, Any]] = None):
        """將查詢結果存入快取
    ...
    async def _check_semantic_cache(self, query: str, context: Optional[Dict[str, Any]] = None) -> Optional[Any]:
        """檢查語義快取（相似查詢）
        ...
        try:
            ...
            for cached_query, (cached_embedding, cached_result, timestamp) in self._semantic_cache.items():
                # 檢查是否過期
                if time.time() - timestamp > self.config.memory_cache_ttl:
                    continue
                
                # 簡單的字串相似度檢查（實際應該使用 embedding 相似度）
                similarity = self._calculate_string_similarity(query_normalized, cached_query)
