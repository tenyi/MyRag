...
class ConfigLoader:
    ...
    def __init__(
        self, config_path: Optional[str] = None, environment: str = "production"
    ):
        """初始化配置載入器
        ...
        logger.info(
            f"配置載入器初始化，環境: {environment}, 配置檔案: {self.config_path}"
        )
    ...
    def _validate_config(self, config: Dict[str, Any]):
        ...
        if "batch_optimization" in config:
            ...
            if batch_config.get("max_batch_size", 0) < batch_config.get(
    ...
    def _create_optimization_config(self, config: Dict[str, Any]) -> OptimizationConfig:
        ...
        return OptimizationConfig(
            ...
            alert_thresholds=monitoring_config.get(
                "alert_thresholds",
                {"cpu_usage": 80.0, "memory_usage": 85.0, "error_rate": 5.0},
            ),
            # 儲存
