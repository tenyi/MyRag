    ...
    try:
        ...
        async def run_validation():
            async with APIValidator(url, timeout) as validator:
                ...
                report_content = validator.generate_validation_report(results, str(report_path))
                ...
                for category, result in results.items():
                    ...
                    console.print(f"   {status_icon} {category.title()}: {result.success_rate:.1f}% ({result.passed}/{result.total})")
    ...
    try:
        ...
        async def run_performance_test():
            async with APIValidator(url) as validator:
                ...
                elif results['success_rate'] >= 95 and results['avg_response_time'] < 3.0:
        ...
        async def run_deploy_check():
            ...
            async with APIValidator(url) as validator:
                ...
                if perf_results["success_rate"] >= 95:
                    console.print(f"   ✅ 效能測試通過 ({perf_results['success_rate']:.1f}%)")
                else:
                    console.print(f"   ⚠️ 效能測試成功率偏低 ({perf_results['success_rate']:.1f}%)")
            ...
            console.print("\\n" + "=" * 50)
            if success:
                console.print("🎉 [bold green]部署前檢查通過！系統準備就緒。[/bold green]")
            else:
                console.print("❌ [bold red]部署前檢查失敗！請修復問題後重試。[/bold red]")
                sys.exit(1)
