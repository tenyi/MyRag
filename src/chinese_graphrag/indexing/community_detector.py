...
class CommunityDetector:
    ...
    def detect_communities(
        ...
        """
        logger.info(
            f"開始社群檢測，實體數: {len(entities)}, 關係數: {len(relationships)}"
        )
        ...
        for community in communities:
            community.summary = self._generate_community_summary(
    ...
    def _dfs_component(
        self, graph: Dict[str, Set[str]], start_node: str, visited: Set[str]
    ) -> Set[str]:
        """
        使用深度優先搜索找到連通分量
    ...
    def _find_community_relationships(
        self, entity_ids: Set[str], relationships: List[Relationship]
    ) -> List[Relationship]:
        """
        找到社群內的關係
    ...
    def _generate_community_title(
        self, entity_ids: Set[str], entities: List[Entity]
    ) -> str:
        """
        生成社群標題
    ...
    def _generate_community_summary(
        ...
        if entity_names:
            summary_parts.append(
                f"這個社群包含 {len(entity_names)} 個實體：{', '.join(entity_names[:5])}"
            )
        ...
        if relationship_descriptions:
            summary_parts.append(
                f"主要關係包括：{'; '.join(relationship_descriptions[:3])}"
            )
    ...
    def _filter_communities(self, communities: List[Community]) -> List[Community]:
        ...
        for community in communities:
            ...
            if (
                ...
                logger.warning(
                    f"社群 {community.id} 太大 ({len(community.entities)} 個實體)，已跳過"
                )

        # 按排名排序，處理 None 值
        filtered_communities.sort(
            key=lambda x: x.rank if x.rank is not None else 0.0, reverse=True
        )
