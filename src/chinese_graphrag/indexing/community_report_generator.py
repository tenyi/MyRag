...
class CommunityReportGenerator:
    ...
    async def _generate_single_community_report(
        ...
        # 分析關係類型分佈
        relationship_type_distribution = self._analyze_relationship_types(
            community_relationships
        )
    ...
    def _collect_related_text_units(
        self, entities: List[Entity], text_units: Dict[str, TextUnit]
    ) -> List[TextUnit]:
        """
        收集與實體相關的文本單元
    ...
    async def _generate_detailed_summary(
        ...
        summary_parts.append(
            f"包含 {len(entities)} 個實體和 {len(relationships)} 個關係"
        )

        # 主要實體
        if entities:
            main_entities = sorted(
                entities, key=lambda x: getattr(x, "rank", 0), reverse=True
            )[:5]
        ...
        if relationships:
            rel_descriptions = [
                rel.description for rel in relationships[:3] if rel.description
            ]
            if rel_descriptions:
