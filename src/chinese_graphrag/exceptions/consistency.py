            ...
            for doc_id in stored_ids.intersection(indexed_ids):
                try:
                    ...
                    if hasattr(vector_store, "get_vector"):
                        ...
                        try:
                            result = asyncio.run(
                                vector_store.get_vector_by_id(
                                    "default", doc_id, include_embedding=True
                                )
                            )
...
class DataConsistencyManager:
    ...
    def run_consistency_check(
        ...
        logger.info(
            f"一致性檢查完成，發現 {len(all_issues)} 個問題，耗時 {execution_time:.2f} 秒"
        )
        return report
    ...
    def auto_fix_issues(self, report: ConsistencyReport) -> Dict[str, Any]:
        ...
        for issue in fixable_issues:
            try:
                ...
                results["failed"] += 1
                results["errors"].append({"issue_id": issue.issue_id, "error": str(e)})
