...
class RetryHandler(ErrorHandler):
    ...
    def handle(self, error: Exception, context: Optional[Dict[str, Any]] = None) -> Any:
        ...
        logger.warning(f"重試錯誤處理 (第 {attempt} 次), 等待 {wait_time:.1f} 秒: {error}")
...
class GlobalErrorHandler:
    ...
    def handle_error(
        ...
        # 沒有合適的處理器時，使用預設策略
        if strategy == HandlingStrategy.ABORT or isinstance(error, ChineseGraphRAGError):
            raise error
...
def setup_error_handling(
    ...
    handler.add_handler(RetryHandler(
