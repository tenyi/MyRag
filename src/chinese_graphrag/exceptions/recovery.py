...
class CheckpointStorage(ABC):
    ...
    @abstractmethod
    def save_checkpoint(
        self, checkpoint_id: str, data: Any, metadata: CheckpointMetadata
    ) -> bool:
        """儲存檢查點"""
        pass
    ...
    ) -> Optional[tuple[Any, CheckpointMetadata]]:
        """載入檢查點"""
        pass
...
class CheckpointManager:
    ...
    def create_checkpoint(
        ...
        with self._lock:
            checkpoint_id = (
                f"cp_{uuid.uuid4().hex[:8]}_{int(datetime.now().timestamp())}"
            )
    ...
    def cleanup_old_checkpoints(
        self, max_age_days: int = 7, keep_minimum: int = 3
    ) -> int:
        """清理過期檢查點"""
        with self._lock:
...
class StateManager:
    ...
    def compare_states(
        self, state1: SystemState, state2: SystemState
    ) -> Dict[str, Any]:
        """比較兩個系統狀態"""
        differences = {
...
class RecoveryManager:
    ...
    def __init__(
        self, checkpoint_manager: CheckpointManager, state_manager: StateManager
    ):
    ...
    def register_recovery_strategy(
        self, error_type: str, strategy: Callable[[Exception, Dict[str, Any]], Any]
    ):
        """註冊恢復策略"""
        with self._lock:
    ...
    def create_recovery_checkpoint(self, operation: str, data: Any) -> str:
        """建立恢復檢查點"""
    ...
    def perform_recovery(
        self, checkpoint_id: str, recovery_steps: Optional[List[Callable]] = None
    ) -> bool:
        """執行恢復操作"""
        with self._lock:
            try:
