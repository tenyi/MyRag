...
class RetryPolicy(ABC):
    ...
    def on_retry(self, attempt: int, exception: Exception, delay: float):
        """重試前的回調"""
        logger.warning(f"第 {attempt} 次重試，延遲 {delay:.2f} 秒。錯誤: {exception}")
...
class CircuitBreakerPolicy(RetryPolicy):
    ...
    def __init__(
        self, failure_threshold: int = 5, recovery_timeout: float = 60.0, **kwargs
    ):
        super().__init__(**kwargs)
...
def async_retry_with_policy(policy: RetryPolicy):
    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 所有重試都失敗了
            logger.error(
                f"非同步重試 {policy.max_attempts} 次後仍然失敗: {last_exception}"
            )
            raise last_exception
...
class RetryManager:
    ...
    def execute_with_retry(
        self, func: Callable, policy_name: str = "default", *args, **kwargs
    ) -> Any:
        """使用指定策略執行函數"""
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """獲取重試統計資訊"""
        return {"policies": list(self.policies.keys()), "stats": dict(self.stats)}
...
def get_retry_manager() -> RetryManager:
    ...
    if _retry_manager is None:
        ...
        _retry_manager.register_policy(
            "fast", FixedDelayPolicy(delay=0.1, max_attempts=2)
        )
        _retry_manager.register_policy(
            "slow", LinearBackoffPolicy(base_delay=2.0, increment=1.0)
        )
