...
class VectorStoreManager:
    ...
    async def initialize(
        self, store_type: Optional[VectorStoreType] = None, **kwargs
    ) -> None:
        """初始化預設向量儲存
    ...
    async def get_store(
        self, store_type: VectorStoreType, store_id: Optional[str] = None, **kwargs
    ) -> VectorStore:
        """取得向量儲存實例
    ...
    async def delete_collection(
        self, name: str, store_type: Optional[VectorStoreType] = None
    ) -> bool:
        """刪除向量集合"""
        store = await self._get_active_store(store_type)
    ...
    async def list_collections(
        self, store_type: Optional[VectorStoreType] = None
    ...
    async def get_index_stats(
        self, collection_name: str, store_type: Optional[VectorStoreType] = None
    ) -> Dict[str, Any]:
        """取得索引統計資訊
    ...
    async def get_vector_by_id(
        ...
        """根據 ID 取得向量資料"""
        store = await self._get_active_store(store_type)
        return await store.get_vector_by_id(
            collection_name, vector_id, include_embedding
        )
    ...
    async def get_statistics(
        ...
        try:
            ...
            dimensions = list(
                set(col.dimension for col in collections if col.dimension > 0)
            )

            return {
                "store_type": (
                    store.store_type.value
                    if hasattr(store.store_type, "value")
                    else str(store.store_type)
                ),
                "collections_count": len(collections),
                "total_vectors": total_vectors,
                "dimensions": dimensions,
                "collections": [
                    {"name": col.name, "dimension": col.dimension, "count": col.count}
