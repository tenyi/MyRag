...
class VectorSearchResult:
    ...
    def __post_init__(self):
        """後處理初始化"""
        # 確保所有列表長度一致
        lengths = [len(self.ids), len(self.distances), len(self.similarities), len(self.metadata)]
        ...
        # 如果提供了重新排序分數，也要檢查長度
        if self.reranking_scores is not None and len(self.reranking_scores) != lengths[0]:
...
class VectorStore(ABC):
    ...
    async def hybrid_search(
        ...
        # 融合結果
        return self._fuse_search_results(results, k, config.reranking_method, config.alpha)
    ...
    def _fuse_search_results(
        ...
        final_distances = []
        final_metadata = []
        final_embeddings = [] if any(c['embedding'] is not None for c in all_candidates.values()) else None
        final_reranking_scores = []
        
        for doc_id, score in sorted_candidates:
            candidate = all_candidates[doc_id]
            final_ids.append(doc_id)
            final_similarities.append(max(candidate['scores']) if candidate['scores'] else 0.0)
            final_distances.append(min(candidate['distances']) if candidate['distances'] else float('inf'))
    ...
    def _fuse_multi_vector_results(
        ...
        for doc_id, candidate in all_candidates.items():
            if fusion_method == "weighted_sum":
                ...
                final_scores[doc_id] = sum(candidate['weighted_similarities']) / len(candidate['weighted_similarities'])
            else:
                final_scores[doc_id] = sum(candidate['weighted_similarities'])
    ...
    def _validate_vectors(
        ...
        if expected_dimension is not None and vectors.shape[1] != expected_dimension:
            raise ValueError(f"向量維度不匹配: 預期 {expected_dimension}, 實際 {vectors.shape[1]}")
    ...
    def _validate_ids_and_vectors(
        ...
        if len(ids) != vectors.shape[0]:
            raise ValueError(f"ID 數量與向量數量不匹配: {len(ids)} vs {vectors.shape[0]}")
        ...
        # 檢查 ID 是否為空
        empty_ids = [i for i, id_val in enumerate(ids) if not id_val or not id_val.strip()]
        if empty_ids:
