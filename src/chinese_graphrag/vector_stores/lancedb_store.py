...
class LanceDBStore(VectorStore):
    ...
    def __init__(self, db_path: str = "./data/lancedb", **kwargs):
        """初始化 LanceDB 儲存
    ...
    async def _sparse_search(
        ...
        try:
            ...
            if query_text is None and sparse_vector:
                # 取權重最高的詞作為關鍵字
                sorted_terms = sorted(
                    sparse_vector.items(), key=lambda x: x[1], reverse=True
                )
                query_text = " ".join(
                    [term for term, _ in sorted_terms[:10]]
                )  # 取前10個詞
            ...
            if search_conditions:
                ...
                if search_filter and search_filter.conditions:
                    ...
                    if filter_conditions:
                        where_clause = (
                            f"({where_clause}) AND ({' AND '.join(filter_conditions)})"
                        )
                ...
                try:
                    ...
                    distances = [
                        1.0 - similarities[i] for i in sorted_indices
                    ]  # 轉換為距離
                    ...
                    if include_embeddings:
                        embeddings = [
                            np.array(results.iloc[i]["vector"]) for i in sorted_indices
                        ]
    ...
    async def optimize_index(
        ...
        try:
            ...
            logger.info(
                f"開始優化集合 {collection_name} 的索引，參數: {default_params}"
            )
        ...
        try:
            ...
            try:
                ...
                if table_stats:
                    stats.update(
                        {
                            "storage_size_mb": table_stats.get("size_bytes", 0)
                            / (1024 * 1024),
                            "index_type": table_stats.get("index_type", "unknown"),
                            "optimization_status": (
                                "optimized"
                                if table_stats.get("has_index", False)
                                else "not_optimized"
                            ),
                        }
                    )
