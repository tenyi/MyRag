...
from .cache import EmbeddingCache, MultiLevelCache, create_embedding_cache
...
# 為索引引擎添加便利方法
async def embed_texts_for_indexing(
...
class EmbeddingManager:
    ...
    def _setup_services_from_config(self, config_or_model):
        ...
        try:
            ...
            if hasattr(config_or_model, "models") and hasattr(
                config_or_model, "model_selection"
            ):
                ...
                logger.info(
                    f"從配置載入 Embedding 服務，模型配置: {list(models.keys())}"
                )
                ...
                for model_key, model_config in models.items():
                    if (
                        hasattr(model_config, "type")
                        and "embedding" in str(model_config.type).lower()
                    ):
                ...
                if self.default_model and self.default_model not in self.services:
                    logger.warning(
                        f"預設 embedding 服務 '{self.default_model}' 不存在，嘗試創建"
                    )
    ...
    def _create_bge_m3_service(self, config):
        ...
        return create_bge_m3_service(
            model_name=config.model,
            device=(
                getattr(config.device, "value", str(config.device))
    ...
    def register_service(
        self, name: str, service: EmbeddingService, set_as_default: bool = False
    ) -> None:
        """註冊 embedding 服務
    ...
    async def _load_service_with_name(
        self, name: str, service: EmbeddingService
    ) -> bool:
        """載入指定服務（內部方法）"""
        try:
    ...
    async def _unload_service_with_name(
        self, name: str, service: EmbeddingService
    ) -> None:
        """卸載指定服務（內部方法）"""
        try:
    ...
    async def embed_texts(
        ...
        if not service.is_loaded:
            ...
            try:
                ...
                logger.warning(
                    f"自動載入 {primary_service_name} 失敗: {e}，將嘗試降級處理"
                )
        ...
        if (
            ...
            and use_gpu_optimization
            and self.memory_optimizer
        ):
            # 檢查記憶體使用情況
        ...
        async with self._semaphore:
            try:
                        ...
                        try:
                            ...
                            if self.enable_monitoring:
                                record_embedding_usage(
                                    ...
                                    device=getattr(
                                        fallback_service, "device", "unknown"
                                    ),
                                    success=True,
                ...
                raise EmbeddingServiceError(
                    f"所有 embedding 服務都失敗，最後錯誤: {error}"
                )
    ...
    async def smart_route_request(
        self, texts: List[str], strategy: str = "fastest", **kwargs
    ) -> EmbeddingResult:
        """智慧路由請求到最適合的服務
        ...
        if strategy == "fastest":
            ...
            # 選擇請求數最少的服務
            best_service = min(
                available_services.items(), key=lambda x: x[1].metrics.total_requests
            )[0]
            ...
            # 簡單的輪詢實作（基於總請求數）
            total_requests = sum(
                s.metrics.total_requests for s in available_services.values()
            )
    ...
    async def preload_cache(
        self, texts: List[str], service_name: Optional[str] = None, batch_size: int = 32
    ) -> Dict[str, Any]:
        """快取預熱"""
        if not self.cache:
            return {"error": "快取未啟用"}
    ...
    async def export_usage_report(
        ...
        """匯出使用量報告"""
        if self.usage_monitor:
            return self.usage_monitor.export_report(
                output_file, time_range_hours, format
            )
        return False
    ...
    def get_comprehensive_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "manager_info": {
                "total_services": len(self.services),
                "loaded_services": sum(
                    1 for s in self.services.values() if s.is_loaded
                ),
                "default_service": self.default_model,
