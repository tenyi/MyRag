...
class BGEM3EmbeddingService(EmbeddingService):
    ...
    async def compute_sparse_embeddings(
        ...
        try:
            # 檢查模型是否支援稀疏表示
            if not hasattr(self.model, "encode") or not hasattr(
                self.model[0], "sparse"
            ):
    ...
    async def compute_colbert_embeddings(self, texts: List[str]) -> List[np.ndarray]:
...
def create_bge_m3_service(
    model_name: str = None, device: str = None, **kwargs
    ...
    """
    return BGEM3EmbeddingService(model_name=model_name, device=device, **kwargs)
