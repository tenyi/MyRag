...
class EmbeddingResult:
    ...
    def __post_init__(self):
        ...
        if actual_dims != self.dimensions:
            logger.warning(
                f"向量維度不匹配: 預期 {self.dimensions}, 實際 {actual_dims}"
            )
...
class ModelMetrics:
    ...
    def update_metrics(
        self, processing_time: float, success: bool = True, memory_usage: float = 0.0
    ):
        """更新效能指標"""
        ...
        if success:
            ...
            self.average_processing_time = self.total_processing_time / (
...
class EmbeddingService(ABC):
    ...
    async def embed_single_text(self, text: str, normalize: bool = True) -> np.ndarray:
        """單一文本向量化
    ...
    async def compute_similarity(
        ...
        if method == "cosine":
            ...
            distances = cdist(
                embeddings1.embeddings, embeddings2.embeddings, metric="euclidean"
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "embedding_dimension": (
                self.get_embedding_dimension() if self.is_loaded else None
            ),
        }
            ...
            try:
                ...
                return {
                    ...
                    "metrics": {
                        ...
                        "average_processing_time": round(
                            self.metrics.average_processing_time, 4
                        ),
                ...
                return {
                    ...
                    "processing_time": round(
                        asyncio.get_event_loop().time() - start_time, 4
                    ),
                }
        ...
        if self.max_sequence_length:
            ...
            for text in valid_texts:
                if len(text) > self.max_sequence_length:
                    ...
                    logger.warning(
                        f"文本超過最大長度限制，已截斷: {len(text)} -> {len(truncated_text)}"
                    )
                else:
