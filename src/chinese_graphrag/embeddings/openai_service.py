...
class OpenAIEmbeddingService(EmbeddingService):
    ...
    def __init__(
        ...
        if dimensions is not None:
            if model_name in ["text-embedding-3-small", "text-embedding-3-large"]:
                ...
                logger.warning(f"模型 {model_name} 不支援自訂維度，忽略 dimensions 參數")
    ...
    async def embed_texts(
        ...
        try:
            ...
            self.estimated_cost += total_tokens * self.model_config.cost_per_token / 1000
            ...
            result = EmbeddingResult(
                ...
                metadata={
                    ...
                    'estimated_cost_usd': round(total_tokens * self.model_config.cost_per_token / 1000, 6),
                    'batch_count': len(batches),
                    'custom_dimensions': self.custom_dimensions
                }
            )
            
            logger.debug(f"向量化完成，耗時: {processing_time:.3f}秒，使用 {total_tokens} tokens")
            return result
    ...
    async def _embed_batch_with_retry(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if "rate_limit" in str(e).lower():
                    ...
                    logger.warning(f"遇到速率限制，等待 {wait_time:.1f} 秒後重試 (嘗試 {attempt + 1}/{self.max_retries + 1})")
        ...
        raise EmbeddingComputeError(f"批次向量化失敗，已重試 {self.max_retries} 次: {last_error}")
