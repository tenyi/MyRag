...
class MemoryCache(EmbeddingCache):
    ...
    async def _evict_if_needed(self, new_entry_size: int) -> None:
        ...
        # 如果還是空間不足，強制淘汰最舊的條目
        while (self.stats['total_size_bytes'] + new_entry_size > self.max_size_bytes 
               and self._cache):
    ...
    async def put(self, key: str, entry: CacheEntry) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.debug(f"磁碟快取存入: {key[:16]}..., 大小: {entry.size_bytes} bytes")
                return True
...
class MultiLevelCache:
    ...
    def __init__(
        ...
        logger.info(f"初始化多層次快取 - 記憶體: {memory_cache_mb}MB, 磁碟: {disk_cache_mb}MB")
    ...
    async def preload_cache(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                if preload_stats['processed_batches'] % 10 == 0:
                    logger.info(f"預熱進度: {preload_stats['processed_batches']} 批次完成")
        ...
        logger.info(f"快取預熱完成: {preload_stats['cached_entries']} 個條目，"
