...
class LocalEmbeddingService(EmbeddingService):
    ...
    def __init__(
        ...
        if model_name in LOCAL_MODELS:
            ...
            actual_max_seq_length = max_sequence_length or self.model_config.max_seq_length
        else:
            # 自訂模型
            if not model_path:
        ...
        logger.info(f"初始化本地 Embedding 服務: {model_name}, 路徑: {actual_model_path}, 裝置: {device}")
    ...
    def _load_model_sync(self) -> SentenceTransformer:
        ...
        try:
            ...
            if Path(self.model_path).exists():
                ...
                model = SentenceTransformer(str(Path(self.model_path).resolve()), **model_kwargs)
            else:
                raise e
