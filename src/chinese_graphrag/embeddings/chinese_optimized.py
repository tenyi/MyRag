...
from .local_models import LocalEmbeddingService, LOCAL_MODELS
...
class ChineseOptimizedEmbeddingService(EmbeddingService):
    ...
    def __init__(
        ...
        if self.config.enable_preprocessing:
            self.text_processor = ChineseTextProcessor(
                chunk_size=self.config.max_segment_length,
                chunk_overlap=50
            )
        else:
    ...
    def _preprocess_texts(self, texts: List[str]) -> List[str]:
        ...
        for text in texts:
            ...
            if self.config.enable_quality_check:
                ...
                chinese_ratio = stats['chinese_char_count'] / stats['char_count'] if stats['char_count'] > 0 else 0
                
                if chinese_ratio < self.config.min_chinese_ratio:
                    logger.warning(f"文本中文比例過低: {chinese_ratio:.2f}, 文本: {text[:50]}...")
            
            # 長文本分段
            if self.config.segment_long_text and len(cleaned_text) > self.config.max_segment_length:
                # 簡單分段策略：按句子分割
                ...
                if segments:
                    ...
                    processed_texts.append(cleaned_text[:self.config.max_segment_length])
            else:
    ...
    async def embed_texts(
        ...
        try:
            ...
            try:
                ...
                result.model_name = f"chinese_optimized_{self.current_service.model_name}"
                ...
                if self.fallback_services:
                    for fallback_name, fallback_service in self.fallback_services.items():
                        try:
                            ...
                            logger.warning(f"備用服務 {fallback_name} 也失敗: {fallback_error}")
                            continue
    ...
    async def evaluate_chinese_quality(
        ...
        try:
            ...
            return {
                ...
                'text_quality': {
                    'average_score': np.mean(text_quality_scores) if text_quality_scores else 0.0,
    ...
    def _calculate_embedding_metrics(self, embeddings: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            'similarity_mean': float(np.mean(similarities)) if len(similarities) > 0 else 0.0,
    ...
    def _calculate_chinese_metrics(
        ...
        # 計算中文文本間的語義相似度分佈
        chinese_texts = [text for text in texts if text and self._is_chinese_dominant(text)]
        chinese_similarity_coherence = 0.0
        
        if len(chinese_texts) > 1:
            # 簡化的語義連貫性評估
            # 實際應用中可以使用更複雜的方法
            chinese_indices = [i for i, text in enumerate(texts) if text and self._is_chinese_dominant(text)]
        ...
        return {
            'chinese_ratio_mean': float(np.mean(chinese_ratios)) if chinese_ratios else 0.0,
            'chinese_ratio_std': float(np.std(chinese_ratios)) if chinese_ratios else 0.0,
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        chinese_info = {
            ...
            'current_service': self.current_service.model_name if self.current_service else None,
            'preprocessing_enabled': self.config.enable_preprocessing,
...
def create_chinese_optimized_service(
    ...
    **kwargs
    ...
    return ChineseOptimizedEmbeddingService(
        config=config,
        device=device
    )