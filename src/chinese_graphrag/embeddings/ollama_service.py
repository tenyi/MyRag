...
from .base import EmbeddingModelType, EmbeddingResult, EmbeddingService, ModelMetrics
...
class OllamaEmbeddingService(EmbeddingService):
    ...
    async def load_model(self) -> None:
        ...
        try:
            ...
            self.is_loaded = True
            logger.info(
                f"成功加載 Ollama 模型: {self.model_name} (維度: {self.dimensions})"
            )
    ...
    async def embed_texts(
        ...
        try:
            ...
            result = EmbeddingResult(
                ...
                model_name=self.model_name,
                dimensions=(
                    embeddings_array.shape[1] if len(embeddings_array) > 0 else 0
                ),
            ...
            logger.debug(
                f"Ollama 向量化完成: {len(texts)} 個文本，耗時 {processing_time:.3f} 秒"
            )
            return result
    ...
    async def _embed_single_text(self, text: str) -> np.ndarray:
        ...
        for attempt in range(self.max_retries):
            try:
                async with aiohttp.ClientSession(
                    ...
                    async with session.post(self.embed_url, json=payload) as response:
                        if response.status == 200:
                            ...
                            raise Exception(
                                f"Ollama API 錯誤 {response.status}: {error_text}"
                            )
                ...
                wait_time = 2**attempt
                logger.warning(
                    f"Ollama API 調用失敗 (嘗試 {attempt + 1}/{self.max_retries}): {e}，{wait_time} 秒後重試"
                )
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(f"{self.base_url}/api/version") as response:
                    ...
                    result = await response.json()
                    logger.info(
                        f"Ollama 服務可用，版本: {result.get('version', 'unknown')}"
                    )
                ...
                async with session.get(self.models_url) as response:
                    if response.status == 200:
                        ...
                        if self.model_name not in model_names:
                            logger.warning(
                                f"模型 {self.model_name} 不在已安裝列表中: {model_names}"
                            )
                            # 嘗試拉取模型
...
def create_ollama_service(
    model_name: str = "bge-m3", base_url: str = "http://localhost:11434", **kwargs
    ...
    """
    return OllamaEmbeddingService(model_name=model_name, base_url=base_url, **kwargs)
