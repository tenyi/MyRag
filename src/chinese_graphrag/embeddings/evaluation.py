...
class EmbeddingEvaluator:
    ...
    async def evaluate_service(
        ...
        for scenario_name, test_func in test_scenarios:
            ...
            try:
                scenario_results = await test_func(service, f"{test_name}_{scenario_name}")
                service_results.extend(scenario_results)
            except Exception as e:
                logger.error(f"{scenario_name} 測試失敗: {e}")
        
        self.results.extend(service_results)
        logger.info(f"完成 {service.model_name} 評估，共 {len(service_results)} 個測試結果")
    ...
    async def _test_text_counts(
        ...
        for text_count in self.config.text_counts:
            ...
            metrics = await self._run_performance_test(
                ...
                test_config={"batch_size": min(32, text_count), "text_count": text_count}
            )
    ...
    async def _run_performance_test(
        ...
        metrics.gpu_memory_after_mb, metrics.gpu_utilization_percent = self._get_gpu_memory_usage()
        
        # 計算統計指標
        if processing_times:
            metrics.total_processing_time = sum(processing_times)
            metrics.average_processing_time = np.mean(processing_times)
            metrics.throughput_texts_per_second = len(texts) / metrics.average_processing_time
        
        metrics.memory_increase_mb = metrics.memory_after_mb - metrics.memory_before_mb
        metrics.error_count = error_count
        metrics.success_rate = (self.config.test_iterations - error_count) / self.config.test_iterations
    ...
    def generate_report(
        ...
        for model_name, model_results in results_by_model.items():
            # 計算模型的平均效能
            avg_processing_time = np.mean([r.average_processing_time for r in model_results])
            avg_throughput = np.mean([r.throughput_texts_per_second for r in model_results])
            ...
            model_summary = {
                ...
                "embedding_dimension": model_results[0].embedding_dimension if model_results else 0,
        ...
        if output_file:
            ...
            output_path = self.output_dir / f"embedding_evaluation_report_{timestamp}.json"
...
class ChineseEmbeddingEvaluator(EmbeddingEvaluator):
    ...
    async def evaluate_chinese_quality(
        ...
        try:
            ...
            # 計算跨領域穩定性
            domain_metrics = await self._calculate_domain_stability(
                service
            )
            ...
            logger.info(f"中文品質評估完成: {service.model_name}, 總分: {overall_score:.3f}")
    ...
    async def _calculate_semantic_consistency(
        ...
        for short_text, long_text in short_long_pairs:
            try:
                ...
                similarity = np.dot(vec_short, vec_long) / (np.linalg.norm(vec_short) * np.linalg.norm(vec_long))
        ...
        if length_consistency_scores:
            metrics["length_consistency_mean"] = float(np.mean(length_consistency_scores))
    ...
    async def _calculate_domain_stability(
        ...
        if len(domain_embeddings) >= 2:
            ...
            if intra_domain_similarities:
                metrics["intra_domain_similarity_mean"] = float(np.mean(intra_domain_similarities))
                metrics["intra_domain_similarity_std"] = float(np.std(intra_domain_similarities))
            
            if inter_domain_similarities:
                metrics["inter_domain_similarity_mean"] = float(np.mean(inter_domain_similarities))
                metrics["inter_domain_similarity_std"] = float(np.std(inter_domain_similarities))
            
            # 領域區分度（領域內相似度應該高於領域間相似度）
            if intra_domain_similarities and inter_domain_similarities:
                domain_separation = np.mean(intra_domain_similarities) - np.mean(inter_domain_similarities)
...
async def chinese_quality_benchmark(
    services: List[EmbeddingService],
    output_dir: str = "chinese_benchmark_results"
) -> Dict[str, Any]:
    """中文品質基準測試的便利函數
