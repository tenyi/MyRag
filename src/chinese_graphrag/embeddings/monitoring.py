...
class UsageRecord:
    ...
    def throughput(self) -> float:
        """吞吐量（項目/秒）"""
        return self.input_count / self.processing_time if self.processing_time > 0 else 0.0
    
    @property
    def tokens_per_second(self) -> float:
        """Token 處理速度（tokens/秒）"""
        return self.input_tokens / self.processing_time if self.processing_time > 0 else 0.0
...
class ModelStats:
    ...
    def average_processing_time(self) -> float:
        """平均處理時間"""
        return self.total_processing_time / self.successful_requests if self.successful_requests > 0 else 0.0
    
    @property
    def average_throughput(self) -> float:
        """平均吞吐量"""
        return self.total_input_count / self.total_processing_time if self.total_processing_time > 0 else 0.0
    
    @property
    def average_memory_per_request(self) -> float:
        """平均每請求記憶體使用量"""
        return self.total_memory_used / self.successful_requests if self.successful_requests > 0 else 0.0
...
class UsageMonitor:
    ...
    def record_usage(
        ...
        **metadata
        ...
        with self._lock:
            ...
            logger.debug(f"記錄使用量: {model_name} - {operation}, "
    ...
    def _check_alerts(self, record: UsageRecord) -> None:
        ...
        # 慢處理警報
        if (self.alert_rules["slow_processing"]["enabled"] and 
            record.success and 
            record.processing_time > self.alert_rules["slow_processing"]["threshold"]):
            self._create_alert(
                AlertLevel.WARNING,
                record.model_name,
                f"處理時間過長: {record.processing_time:.2f}s",
                {"processing_time": record.processing_time, "operation": record.operation}
            )
        
        # 高記憶體使用警報
        if (self.alert_rules["high_memory_usage"]["enabled"] and 
            record.memory_used > self.alert_rules["high_memory_usage"]["threshold"]):
            self._create_alert(
                AlertLevel.WARNING,
                record.model_name,
            ...
            if error_rate > threshold:
                self._create_alert(
                    ...
                    f"錯誤率過高: {error_rate:.1%}",
                    {
                        ...
                        "window_minutes": self.alert_rules["high_error_rate"]["window_minutes"]
                    }
                )
    ...
    def get_model_stats(self, model_name: Optional[str] = None) -> Union[ModelStats, Dict[str, ModelStats]]:
        """取得模型統計資訊
    ...
    def get_usage_summary(
        ...
        with self._lock:
            ...
            filtered_records = [
                r for r in self.usage_records
                if r.timestamp >= start_time and (not model_name or r.model_name == model_name)
            ]
            ...
            total_processing_time = sum(r.processing_time for r in filtered_records if r.success)
            total_memory_used = sum(r.memory_used for r in filtered_records if r.success)
            
            # 按模型分組統計
            model_breakdown = defaultdict(lambda: {
                'requests': 0, 'successful': 0, 'failed': 0,
                'input_count': 0, 'processing_time': 0.0, 'memory_used': 0.0
            })
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_throughput": total_input_count / total_processing_time if total_processing_time > 0 else 0.0
                },
    ...
    def get_performance_trends(
        ...
        with self._lock:
            ...
            records = [
                r for r in self.usage_records
                if r.timestamp >= start_time and r.model_name == model_name and r.success
            ]
            ...
            for record in records:
                if granularity == "hour":
                    bucket_key = datetime.fromtimestamp(record.timestamp).strftime("%Y-%m-%d %H:00")
                else:  # day
                    bucket_key = datetime.fromtimestamp(record.timestamp).strftime("%Y-%m-%d")
            ...
            for time_key in sorted(time_buckets.keys()):
                ...
                avg_throughput = total_input_count / total_processing_time if total_processing_time > 0 else 0
        ...
        try:
            ...
            if format == "json":
                ...
                with open(output_file, 'w', newline='', encoding='utf-8') as f:
                    ...
                    writer.writerow(["模型名稱", "請求數", "成功數", "失敗數", "輸入數量", "處理時間", "記憶體使用"])
