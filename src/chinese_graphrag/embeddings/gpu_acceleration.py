...
class DeviceManager:
    ...
    def _detect_devices(self) -> None:
        ...
        if torch.cuda.is_available():
            ...
            for i in range(cuda_count):
                ...
                logger.info(
                    f"偵測到 CUDA 裝置: {device_name} - {torch.cuda.get_device_name(i)}"
                )
    ...
    def get_optimal_device(
        self, memory_required_mb: int = 0, prefer_gpu: bool = True
    ) -> str:
        """取得最佳裝置
                ...
                if device.startswith("cuda:"):
                    ...
                    if gpu_id in self.gpu_info:
                        ...
                        if (
                            ...
                            and gpu.memory_free < memory_required_mb
                        ):
                            continue
    ...
    def get_device_info(self, device: str) -> Dict[str, Any]:
        ...
        if device == "mps":
            return {
                ...
                "type": "MPS",
                "available": (
                    torch.backends.mps.is_available() if TORCH_AVAILABLE else False
                ),
            }
...
class MemoryOptimizer:
    ...
    def _should_cleanup(self) -> bool:
        ...
        return (
            stats.system_usage_ratio > self.memory_threshold
            or stats.gpu_usage_ratio > self.memory_threshold
        )
    ...
    async def cleanup_memory(self, aggressive: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"記憶體清理完成，釋放: 系統 {system_freed}MB, GPU {gpu_freed}MB, "
...
class BatchProcessor:
    ...
    def __init__(
        ...
        self.batch_history: List[Tuple[int, float, bool]] = (
            []
        )  # (batch_size, processing_time, success)

        logger.info(f"初始化批次處理器，初始批次大小: {initial_batch_size}")

    def calculate_optimal_batch_size(
        self, device: str, estimated_memory_per_item: int = 10  # MB per item
    ) -> int:
        """計算最佳批次大小
        ...
        # 基於歷史效能調整
        performance_adjusted_size = self._adjust_based_on_history(
            memory_based_batch_size
        )

        # 最終批次大小
        optimal_size = max(
            self.min_batch_size, min(self.max_batch_size, performance_adjusted_size)
        )
    ...
    def _adjust_based_on_history(self, base_size: int) -> int:
        ...
        # 計算成功率
        success_rate = sum(1 for _, _, success in recent_history if success) / len(
            recent_history
        )
        ...
        # 如果成功率高且處理時間合理，可以嘗試增大批次大小
            ...
            avg_time = sum(
                time for _, time, success in recent_history if success
            ) / max(1, sum(1 for _, _, success in recent_history if success))
    ...
    def record_batch_result(
        ...
        logger.debug(
            f"記錄批次結果: 大小={batch_size}, 時間={processing_time:.2f}s, "
            f"成功={success}"
        )
    ...
    def get_batch_stats(self) -> Dict[str, Any]:
        ...
        if not self.batch_history:
            return {"total_batches": 0, "current_batch_size": self.current_batch_size}

        successful_batches = [
            (size, time) for size, time, success in self.batch_history if success
        ]
        failed_batches = [
            (size, time) for size, time, success in self.batch_history if not success
        ]
...
def create_batch_processor(**kwargs) -> BatchProcessor:
    ...
    return BatchProcessor(
        device_manager=device_manager, memory_optimizer=memory_optimizer, **kwargs
    )
