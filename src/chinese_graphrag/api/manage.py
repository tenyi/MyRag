...
async def validate_api(
    base_url: str = "http://localhost:8000", report_file: Optional[str] = None
):
    """驗證 API 端點。
...
async def deploy_check(base_url: str):
    ...
    try:
        perf_results = await performance_test(
            base_url=base_url, endpoint="/health", concurrent=5, duration=10
        )
...
範例：
  ...
  %(prog)s deploy-check --url http://...  # 部署前檢查
    ...
    docs_parser.add_argument(
        "--output", "-o", default="docs/api", help="輸出目錄 (預設: docs/api)"
    )
    ...
    validate_parser.add_argument("--report", "-r", help="驗證報告檔案路徑")
    ...
    perf_parser.add_argument(
        "--endpoint", "-e", default="/health", help="測試端點 (預設: /health)"
    )
    perf_parser.add_argument(
        "--concurrent", "-c", type=int, default=10, help="並發數 (預設: 10)"
    )
    perf_parser.add_argument(
    ...
    server_parser.add_argument(
        "--host", default="0.0.0.0", help="綁定主機 (預設: 0.0.0.0)"
    )
    server_parser.add_argument(
        "--port", "-p", type=int, default=8000, help="綁定端口 (預設: 8000)"
    )
    server_parser.add_argument(
        "--reload", action="store_true", help="啟用自動重載（開發模式）"
    )
    server_parser.add_argument(
        "--workers", "-w", type=int, default=1, help="工作進程數 (預設: 1)"
    )
