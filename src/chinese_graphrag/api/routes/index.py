...
from ..models import (
    IndexRequest, IndexResponse, IndexStatus, DataResponse,
...
async def cancel_index_task(task_id: str) -> DataResponse:
    ...
    try:
        ...
        if task_info["status"] not in [ResponseStatus.PROCESSING, ResponseStatus.PENDING]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def cleanup_index_tasks(
    ...
    try:
        ...
        for task_id, task_info in active_tasks.items():
            task_age_hours = (current_time - task_info["start_time"]).total_seconds() / 3600
    ...
    try:
        ...
        if failed_count == 0:
            await _complete_task_successfully(
                task_id,
                f"索引建立完成，共處理 {processed_count} 個檔案"
            )
        else:
            await _complete_task_successfully(
                task_id,
