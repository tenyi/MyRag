...
@router.get(
    "/monitoring/metrics", response_model=MonitoringResponse, summary="取得系統監控指標"
)
...
@router.get(
    "/monitoring/history", response_model=DataResponse, summary="取得監控歷史資料"
)
async def get_metrics_history(
    hours: int = 1, interval_minutes: int = 5
) -> DataResponse:
    """取得指定時間範圍內的監控歷史資料。
    ...
    try:
        ...
        # 過濾指定時間範圍的資料
        filtered_history = [
            m for m in metrics_history if start_time <= m["timestamp"] <= current_time
        ]
        ...
        if interval_minutes > 0:
            ...
            for entry in sorted(filtered_history, key=lambda x: x["timestamp"]):
                if (
                    last_sampled_time is None
                    or (entry["timestamp"] - last_sampled_time).total_seconds()
                    >= interval_minutes * 60
                ):
...
async def get_alerts(severity: Optional[str] = None, limit: int = 50) -> DataResponse:
    """取得系統警告和異常事件。
    ...
    try:
        ...
        return create_success_response(
            ...
            meta={"severity_filter": severity, "limit": limit},
        )
...
@router.get(
    "/monitoring/performance", response_model=DataResponse, summary="取得效能統計"
)
async def get_performance_stats() -> DataResponse:
    ...
    try:
        ...
        if total_queries > 0:
            success_rate = (
                application_stats["successful_queries"] / total_queries
            ) * 100
        ...
        if recent_metrics:
            ...
            memory_values = [
                m["system_metrics"]["memory_usage"] for m in recent_metrics
            ]
        ...
        return create_success_response(
            data=performance_stats, message="成功取得效能統計資訊"
        )
...
async def run_system_test(
    test_type: str = "basic", settings: Settings = Depends()
) -> DataResponse:
    """執行系統測試以驗證各項功能。
    ...
    try:
        ...
        return create_success_response(
            data=result, message=f"系統測試完成：{overall_status}"
        )
...
async def _run_basic_tests() -> Dict[str, Any]:
    ...
    try:
        logger.info("測試日誌系統")
        results["logging"] = {"status": "passed", "message": "日誌系統工作正常"}
    except Exception as e:
        results["logging"] = {
            "status": "failed",
...
def update_connection_count(delta: int):
    ...
    Args:
        delta: 連接數變化量
    """
    application_stats["active_connections"] = max(
        0, application_stats["active_connections"] + delta
    )
