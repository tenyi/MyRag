...
class APIValidator:
    ...
    async def validate_endpoint(self, 
        ...
        try:
            ...
            error_message = None if success else f"預期狀態碼 {expected_status}，實際 {response.status_code}"
    ...
    def _calculate_validation_result(self, category: str, results: List[TestResult]) -> ValidationResult:
        """計算驗證結果統計。
    ...
    def generate_validation_report(self, 
        ...
        if overall_success_rate >= 90:
            report_lines.append("🎉 **優秀**: API 整體表現良好，所有主要功能都正常運作。")
        elif overall_success_rate >= 80:
            report_lines.append("👍 **良好**: API 大部分功能正常，建議修復失敗的測試案例。")
        elif overall_success_rate >= 60:
            report_lines.append("⚠️ **需要改進**: API 存在一些問題，建議優先修復關鍵功能。")
        else:
            report_lines.append("🚨 **需要立即處理**: API 存在嚴重問題，建議暫停部署並修復所有錯誤。")
