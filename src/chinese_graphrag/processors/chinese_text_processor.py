...
class ChineseTextProcessor:
    ...
    def _load_stopwords(self, stopwords_path: Optional[str] = None) -> Set[str]:
        ...
        default_stopwords = {
            ...
            '她', '它', '們', '這個', '那個', '這些', '那些', '什麼', '怎麼',
            '為什麼', '哪裡', '哪個', '誰', '什麼時候', '怎樣', '多少',
            '啊', '呢', '吧', '嗎', '呀', '哦', '哎', '唉', '嗯', '嘿',
            '但是', '可是', '然而', '不過', '雖然', '儘管', '如果', '假如',
            '因為', '所以', '由於', '因此', '而且', '並且', '或者', '還是',
            '以及', '以及', '等等', '之類', '比如', '例如', '譬如', '諸如',
        }
        ...
        if stopwords_path:
            try:
                ...
                logger.info(f"載入停用詞檔案: {stopwords_path}, 共 {len(file_stopwords)} 個詞")
            except Exception as e:
    ...
    def segment_text(
        ...
        if keep_pos:
            ...
            # 普通分詞
            words = [word.strip() for word in jieba.cut(cleaned_text) 
                    if word.strip() and word not in self.chinese_punctuation]
    ...
    def evaluate_text_quality(self, text: str) -> Dict[str, any]:
        ...
        # 2. 中文比例評估 (0-1分)
        chinese_ratio = stats['chinese_char_count'] / char_count if char_count > 0 else 0
        ...
        # 5. 標點符號使用評估 (0-1分)
        punctuation_ratio = stats['punctuation_count'] / char_count if char_count > 0 else 0
        ...
        # 6. 文本連貫性評估 (0-1分)
        # 簡單的連貫性檢查：檢查連接詞的使用
        coherence_words = ['但是', '然而', '因此', '所以', '而且', '並且', '另外', '此外', '首先', '其次', '最後', '總之']
        coherence_count = sum(1 for word in coherence_words if word in text)
        
        if sentence_count > 5:
            expected_coherence = sentence_count * 0.1  # 預期每10句話有一個連接詞
            coherence_ratio = coherence_count / expected_coherence if expected_coherence > 0 else 0
