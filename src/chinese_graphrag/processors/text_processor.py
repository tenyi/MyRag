...
class TextProcessor(BaseDocumentProcessor):
    ...
    def extract_content(self, file_path: str) -> str:
        ...
        try:
            ...
            if not content:
                raise ContentExtractionError(file_path, "檔案內容為空")

            return content

        except UnicodeDecodeError as e:
            raise ContentExtractionError(file_path, f"編碼解碼失敗: {str(e)}")
        except Exception as e:
            raise ContentExtractionError(file_path, f"讀取檔案失敗: {str(e)}")
...
class MarkdownProcessor(BaseDocumentProcessor):
    ...
    def extract_content(self, file_path: str) -> str:
        ...
        try:
            ...
            if not text_content:
                raise ContentExtractionError(file_path, "轉換後的內容為空")
            ...
            if isinstance(e, ContentExtractionError):
                raise
            raise ContentExtractionError(file_path, f"Markdown 處理失敗: {str(e)}")
    ...
    def extract_raw_content(self, file_path: str) -> str:
        ...
        try:
            ...
            raise ContentExtractionError(file_path, f"讀取 Markdown 檔案失敗: {str(e)}")
