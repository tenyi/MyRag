...
from chinese_graphrag.models import Entity, QueryResult, Relationship, TextUnit
            ...
            for entity in all_entities:
                if entity not in target_entities:
                    ...
                    if keywords.intersection(entity_chars):
                        target_entities.append(entity)
                        print(
                            f"關鍵詞字符匹配: {keywords.intersection(entity_chars)} -> {entity.name}"
                        )
                    ...
                    if entity.description:
                        ...
                        if keywords.intersection(desc_words):
                            target_entities.append(entity)
                            print(
                                f"描述匹配: {keywords.intersection(desc_words)} -> {entity.name}"
                            )
                ...
                for entity in all_entities:
                    if entity not in target_entities:
                        ...
                        if distance <= min(2, len(query_entity) // 2):
                            target_entities.append(entity)
                            print(
                                f"編輯距離匹配 (距離={distance}): {query_entity} -> {entity.name}"
                            )
            ...
            try:
                ...
                async def vector_search():
                    try:
                        ...
                        # 為查詢生成 embedding
                        query_result = await embedding_manager.embed_texts(
                        ...
                        for i, entity_id in enumerate(
                            ...
                            if entity_id in entity_id_map:
                                ...
                                if entity not in target_entities:
                                    ...
                                    print(
                                        f"向量相似匹配: {query} -> {entity.name} (相似度: {search_results.similarities[i] if hasattr(search_results, 'similarities') else 'N/A'})"
                                    )
...
class TextunitRetriever:
    ...
    def _calculate_text_unit_relevance(
        self, text_unit: TextUnit, entity_ids: Set[str]
    ) -> float:
        """計算文本單元相關性分數"""
        # 這裡需要根據實際的文本單元結構來實作
        # 假設文本單元有某種方式記錄相關實體
...
class EntityFocusedStrategy(LocalSearchStrategy):
    ...
    def _build_entity_knowledge_graph(
        ...
        for rel in relationships:
            if (
                rel.source_entity_id in target_entity_ids
                or rel.target_entity_id in target_entity_ids
            ):
    ...
    def _collect_evidence_texts(
        self, target_entities: List[Entity], text_units: List[TextUnit]
    ) -> List[str]:
        """收集證據文本"""
        evidence = []
...
請開始回答："""
    ...
    def _calculate_entity_confidence(
        self, context: LocalSearchContext, target_entities: List[Entity]
    ) -> float:
        """計算實體信心度"""
...
class RelationshipFocusedStrategy(LocalSearchStrategy):
    ...
    def _identify_key_relationships(
        self, context: LocalSearchContext
    ) -> List[Relationship]:
        """識別關鍵關係"""
    ...
    def _build_relationship_network(
        ...
        for rel in relationships:
            if (
                rel.source_entity_id in network["nodes"]
                and rel.target_entity_id in network["nodes"]
            ):
    ...
    def _collect_relationship_evidence(
        self, relationships: List[Relationship], text_units: List[TextUnit]
    ) -> List[str]:
        """收集關係證據"""
        evidence = []
    ...
    def _build_relationship_search_context(
        ...
        if network["edges"]:
            ...
            for edge in network["edges"][:10]:
                ...
                context_parts.append(
                    f"- {source_name} → {target_name}: {edge['description']}"
                )
...
請基於關係分析回答查詢："""
    ...
    def _calculate_relationship_confidence(
        self, context: LocalSearchContext, key_relationships: List[Relationship]
    ) -> float:
        """計算關係信心度"""
