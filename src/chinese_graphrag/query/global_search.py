...
from chinese_graphrag.models import Community, Entity, QueryResult, Relationship
...
class CommunityBasedStrategy(GlobalSearchStrategy):
    ...
    async def search(
        ...
        result = GlobalSearchResult(
            ...
            confidence=self._calculate_confidence(
                context.analysis, selected_communities
            ),
            sources=sources,
    ...
    def _build_search_context(
        self, query: str, analysis: QueryAnalysis, communities: List[Community]
    ) -> str:
        """構建搜尋上下文"""
        context_parts = []
        ...
        for i, community in enumerate(communities, 1):
            ...
            if community.entities:
                ...
                if len(community.entities) > 5:
                    context_parts.append(
                        f"（還有 {len(community.entities) - 5} 個實體）"
                    )
...
請開始回答："""
    ...
    def _calculate_confidence(
        self, analysis: QueryAnalysis, communities: List[Community]
    ) -> float:
        """計算回答的信心度"""
...
class HierarchicalStrategy(GlobalSearchStrategy):
    ...
    async def search(
        ...
        level_results = []
        max_level = (
            max(comm.level for comm in context.communities)
        ...
        for level in range(max_level, 0, -1):
            level_communities = [
                comm for comm in context.communities if comm.level == level
            ]
    ...
    async def _search_at_level(
        ...
        selected = [
            comm for comm, score in ranked_communities[:max_communities_per_level]
        ]
...
請提供這個層級的相關資訊和分析："""
        ...
        try:
            partial_answer = await llm_manager.generate(
                prompt, TaskType.GLOBAL_SEARCH, max_tokens=800, temperature=0.7
            )
        except Exception as e:
...
綜合回答："""
        ...
        try:
            final_answer = await llm_manager.generate(
                prompt, TaskType.GLOBAL_SEARCH, max_tokens=2000, temperature=0.6
            )
    ...
    def _calculate_hierarchical_confidence(
        ...
        community_bonus = (
            sum(len(result["communities"]) for result in level_results) * 0.05
        )
