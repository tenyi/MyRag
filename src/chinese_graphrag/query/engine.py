...
class QueryEngine:
                ...
                with open(relationships_file, 'r', encoding='utf-8') as f:
                    ...
                    for rel_id, rel_info in relationships_data.items():
                        relationship = Relationship(
                            ...
                            relationship_type=rel_info.get('relationship_type', 'related_to'),
    ...
    async def _load_from_vector_db_async(self):
        ...
        try:
            ...
            logger.info(f"從向量資料庫載入資料完成: {len(self.indexer.entities)} 個實體, {len(self.indexer.text_units)} 個文本單元, {len(self.indexer.communities)} 個社群")
    ...
    async def _load_entities_from_vector_db(self):
        ...
        try:
            ...
            # 使用隨機向量搜尋來獲取所有實體
            query_vector = np.random.rand(entities_collection.dimension).astype(np.float32)
            ...
            for i, entity_id in enumerate(search_result.ids):
                ...
                embedding = search_result.embeddings[i] if search_result.embeddings else None
    ...
    async def _load_text_units_from_vector_db(self):
        ...
        try:
            ...
            # 使用隨機向量搜尋來獲取所有文本單元
            query_vector = np.random.rand(text_units_collection.dimension).astype(np.float32)
    ...
    async def _load_communities_from_vector_db(self):
        ...
        try:
            ...
            # 使用隨機向量搜尋來獲取所有社群
            query_vector = np.random.rand(communities_collection.dimension).astype(np.float32)
    ...
    async def _load_text_units_from_vector_db(self):
        ...
        try:
            ...
            # 轉換為 TextUnit 物件 - 修正屬性名稱
            for i in range(len(search_result.ids)):
                unit_id = search_result.ids[i]
                metadata = search_result.metadata[i]
                embedding = search_result.embeddings[i] if search_result.embeddings else None
    ...
    async def _load_communities_from_vector_db(self):
        ...
        try:
            ...
            # 轉換為 Community 物件 - 修正屬性名稱
            for i in range(len(search_result.ids)):
                community_id = search_result.ids[i]
                metadata = search_result.metadata[i]
                embedding = search_result.embeddings[i] if search_result.embeddings else None
