...
class LLMMetrics:
    ...
    def update_metrics(
        self, success: bool, tokens: int, cost: float, response_time: float
    ):
        """更新指標"""
...
class LLMAdapter(ABC):
    ...
    @abstractmethod
    async def generate(self, prompt: str, task_type: TaskType, **kwargs) -> str:
        """生成回應"""
        pass
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            await asyncio.wait_for(
                self.llm.async_generate(test_prompt, max_tokens=10), timeout=10.0
            )
            self._is_healthy = True
            return True
...
class LLMManager:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        for attempt in range(max_retries + 1):
            try:
                logger.debug(
                    f"使用 {adapter.config.model} 生成回應 (嘗試 {attempt + 1})"
                )
    ...
    def _select_best_adapter(
        self, task_type: TaskType, preferred_provider: Optional[str] = None
    ) -> Optional[LLMAdapter]:
        """選擇最佳適配器"""
        # 過濾支援該任務類型且健康的適配器
        candidates = [
        ...
        def score_adapter(adapter: LLMAdapter) -> float:
            ...
            if adapter.metrics.avg_response_time > 0:
                response_time_penalty = min(
                    adapter.metrics.avg_response_time / 10.0, 0.5
                )
            else:
    ...
    def _get_fallback_adapter(
        self, failed_adapter: LLMAdapter, task_type: TaskType
    ) -> Optional[LLMAdapter]:
        """獲取降級適配器"""
        candidates = [
            adapter
            ...
            if (
                ...
                and adapter.supports_task(task_type)
                and adapter.is_healthy
            )
        ]
    ...
    def get_metrics(self) -> Dict[str, LLMMetrics]:
        """獲取所有適配器的效能指標"""
        return {name: adapter.metrics for name, adapter in self.adapters.items()}
