    ...
    def _parse_segmentation_response(self, response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if (
                ...
                and not line.startswith("輸出：")
                and not line.startswith("結果：")
            ):
                # 檢查是否包含中文字符
    ...
    async def extract_keywords_with_llm(
        ...
        for word in words:
            ...
            if (
                ...
                and not re.match(self.punctuation_pattern, word)
...
class EntityExtractor:
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        for word in all_tokens:
            ...
            if (
                len(word) >= 2
                and word not in found_entities
                and self._is_likely_entity(word)
            ):
...
class ChineseQueryProcessor:
    ...
    async def process_query_with_llm_segmentation(self, query: str) -> QueryAnalysis:
        ...
        try:
            ...
            if keywords:
                preprocessing_notes.append(
                    f"使用 LLM 分詞提取到 {len(keywords)} 個關鍵詞"
                )
        except Exception as e:
            ...
            if keywords:
                preprocessing_notes.append(
                    f"回退到 jieba 提取到 {len(keywords)} 個關鍵詞"
                )
        ...
        logger.info(
            f"LLM 分詞查詢分析完成: {query_type.value} ({overall_confidence:.2f})"
        )
        return analysis
            ...
            if self.llm_manager:
                ...
                for word in llm_segments:
                    ...
                    if (
                        len(word) >= 2
                        and word not in found_entities
                        and self.entity_extractor._is_likely_entity(word)
                    ):
                        entities.append(word)
                        found_entities.add(word)
            else:
    ...
    def _suggest_llm_task(
        self, query_type: QueryType, intent: QueryIntent, entities: List[str]
    ) -> TaskType:
        """建議適合的 LLM 任務類型"""
    ...
    def _get_search_strategy(self, analysis: QueryAnalysis) -> Dict[str, Any]:
        ...
        strategy = {
            "primary_method": (
                ...
                in [QueryType.GLOBAL_SEARCH, QueryType.SUMMARY, QueryType.COMPARISON]
                else "local_search"
            ),
            "entity_focus": len(analysis.entities) > 0,
            "requires_reasoning": analysis.intent
            in [
                QueryIntent.EXPLANATION,
                QueryIntent.PROBLEM_SOLVING,
                QueryIntent.PLANNING,
