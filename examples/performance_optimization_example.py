...
class PerformanceOptimizationDemo:
    ...
    async def demo_batch_optimization(self):
        ...
        # 準備測試資料
        test_texts = [
            f"這是測試文本 {i}，用於演示批次處理優化功能。" for i in range(100)
        ]
    ...
    async def demo_query_optimization(self):
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            result = await self.optimizer_manager.optimize_query(
                query=query, query_func=simulate_query_task
            )
    ...
    async def demo_cost_optimization(self):
        ...
        for prompt in test_prompts:
            ...
            print(
                f"   📊 Token 使用: {result['input_tokens']} 輸入 + {result['output_tokens']} 輸出"
            )
    ...
    async def demo_performance_monitoring(self):
        ...
        # 取得當前效能指標
        current_metrics = (
            self.optimizer_manager.performance_monitor.get_current_metrics()
        )
        if current_metrics:
            print(f"📊 當前效能指標:")
        ...
        # 取得效能統計
        performance_stats = (
            self.optimizer_manager.performance_monitor.get_performance_stats(5)
        )
        if performance_stats:
            ...
            if "cpu" in system_stats:
                ...
                print(
                    f"   CPU - 平均: {cpu_stats.get('avg', 0):.1f}%, "
            ...
            if "memory" in system_stats:
                ...
                print(
                    f"   記憶體 - 平均: {memory_stats.get('avg_mb', 0):.1f}MB, "
            ...
            if trends:
                print(
                    f"   趨勢 - CPU: {trends.get('cpu_trend', 'unknown')}, "
    ...
    async def demo_benchmark_testing(self):
        ...
        test_configs = [
            ...
            {
                "name": "批量嵌入",
                "func": lambda: simulate_embedding_task(
                    ["文本1", "文本2", "文本3", "文本4", "文本5"]
                ),
                "params": {},
            },
            ...
            {
                "name": "複雜查詢",
                "func": lambda: simulate_query_task(
                    "這是一個更複雜的查詢，需要更多處理時間和資源"
                ),
                "params": {},
            },
        ]
        ...
        results = await self.optimizer_manager.run_performance_benchmark(
            test_configs=test_configs, iterations=10
        )
        ...
        for test_name, result in results.items():
            ...
            if result.latency_percentiles:
                print(
                    f"   延遲分佈 - P50: {result.latency_percentiles.get('p50', 0):.2f}ms, "
